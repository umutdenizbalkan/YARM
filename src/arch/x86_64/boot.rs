...
pub fn bootstrap_first_user_task(
    ...
    let supervisor_aei: Option<(_, usize, usize)> = if let Some(ref sup_bytes) = supervisor_image {
        let sup_elf_info =
            yarm_srv_common::elf::ElfImageInfo::parse(1, sup_bytes).map_err(|e| {
                crate::yarm_log!("BOOTSTRAP_ERROR parse_supervisor_elf_header: {:?}", e);
    ...
    let pm_aei: Option<(_, usize, usize)> = if let Some(ref pm_bytes) = pm_image {
        let pm_elf_info = yarm_srv_common::elf::ElfImageInfo::parse(2, pm_bytes).map_err(|e| {
            crate::yarm_log!("BOOTSTRAP_ERROR parse_pm_elf_header: {:?}", e);
            crate::kernel::boot::KernelError::WrongObject
        })?;
    ...
    // Stage 199D-WA3B: RESERVE, do not register. The bootstrap dependency is unchanged — the
    // capability grants below still need a destination CNode and kernel-side provisioning, and a
    // reservation owns exactly those — but a reservation is NOT a live task, so the spawns below
    // consume an exact one-shot token instead of overwriting whatever occupies the TID.
    ...
    // and init runs byte-identically. This is not on the D6/CR3/TSS/PF path.
    ...
    // Active only when BOTH proof knobs are set; queued-split + rollback proof
    // boots (base knob only) leave slot 13 zero.
...
pub fn enter_dispatched_user_task_if_available(
    ...
    if context.instruction_ptr.0 != 0 && context.stack_ptr.0 != 0 {
        ...
        #[allow(unreachable_code)]
        {
            ...
            crate::yarm_log!("X86_ENTER_USER_RSP stack=0x{:x}", context.stack_ptr.0,);
...
fn capture_pvh_command_line(start_info_ptr: usize) {
    ...
    let virtual_address =
        crate::arch::platform_layout::KERNEL_BOOTSTRAP_VIRT_BASE.saturating_add(physical);
    ...
    let source = unsafe { core::slice::from_raw_parts(virtual_address as *const u8, read_len) };
