...
struct IpcSubsystem {
    ...
    direct_mailbox: [Option<(usize, Message)>; MAX_TASKS],
}
    ...
    pub fn init() -> Result<KernelState, KernelError> {
        ...
        let mut state = KernelState {
            ...
            ipc: IpcSubsystem {
                ...
                direct_mailbox: [const { None }; MAX_TASKS],
            },
            tcbs: [None; MAX_TASKS],
            memory: MemorySubsystem {
...
impl KernelState {
    ...
    fn deliver_direct_mailbox(
        &mut self,
        endpoint_idx: usize,
        waiter_tid: ThreadId,
        msg: Message,
    ) -> Result<bool, KernelError> {
        let slot_idx = waiter_tid.0 as usize;
        if slot_idx >= MAX_TASKS {
            return Ok(false);
        }
        if self.ipc.direct_mailbox[slot_idx].is_some() {
            return Ok(false);
        }

        self.ipc.direct_mailbox[slot_idx] = Some((endpoint_idx, msg));
        self.ipc.endpoint_waiters[endpoint_idx] = None;
        let tcb = self.tcb_mut(waiter_tid.0).ok_or(KernelError::TaskMissing)?;
        tcb.status = TaskStatus::Runnable;
        self.scheduler
    ...
    pub fn ipc_send(&mut self, send_cap: CapId, msg: Message) -> Result<(), KernelError> {
        ...
        if let Some(waiter_tid) = self.ipc.endpoint_waiters[endpoint_idx] {
            if msg.is_fastpath_inline()
                && self.deliver_direct_mailbox(endpoint_idx, waiter_tid, msg)?
            {
                return Ok(());
            }
        }
    ...
    pub fn ipc_recv(&mut self, recv_cap: CapId) -> Result<Option<Message>, KernelError> {
        ...
        if let Some(current_tid) = self.scheduler.current_tid() {
            let slot_idx = current_tid as usize;
            if slot_idx < MAX_TASKS
                && self.ipc.direct_mailbox[slot_idx]
                    .as_ref()
                    .is_some_and(|(mail_ep, _)| *mail_ep == endpoint_idx)
            {
                let (_, msg) = self.ipc.direct_mailbox[slot_idx]
                    .take()
                    .expect("checked mailbox message");
                return Ok(Some(msg));
            }
        }
...
mod tests {
    ...
    #[test]
    fn synchronous_endpoint_still_sends_asynchronously() {
        let mut state = Bootstrap::init().expect("init");
        state.register_task(1).expect("register task 1");
        state.scheduler.enqueue(1).expect("queue task 1");
        ...
        state
            .ipc_send(send_cap, msg)
            .expect("async send on sync endpoint");
    ...
    #[test]
    fn fastpath_inline_message_uses_direct_mailbox_without_queueing() {
        let mut state = Bootstrap::init().expect("init");
        state.register_task(1).expect("register task 1");
        state.scheduler.enqueue(1).expect("queue task 1");
        ...
        let msg = Message::new(1, b"tiny").expect("msg");
        state.ipc_send(send_cap, msg).expect("fastpath send");

        assert_eq!(state.ipc.endpoints[eid].as_ref().expect("ep").queued(), 0);
        state.set_current_cpu(CpuId(0)).expect("cpu0");
        state.yield_current().expect("switch back");
        let recv = state
            .ipc_recv(recv_cap)
            .expect("recv")
            .expect("mailbox msg");
        assert_eq!(recv.as_slice(), b"tiny");
    }
}
