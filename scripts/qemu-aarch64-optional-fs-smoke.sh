...
if ! command -v "$QEMU_BIN" >/dev/null 2>&1; then
  echo "[warn] qemu binary not available ($QEMU_BIN); skipping optional-FS smoke"
  exit 0
fi

LOGFILE=${LOGFILE:-qemu-aarch64-optional-fs.log}
rm -f "$LOGFILE"
...
echo "[info] qemu-aarch64-optional-fs-smoke: running $QEMU_BIN ${QEMU_ARGS[*]}"
...
if [[ -f "$LOGFILE" ]]; then
  ...
  if [[ "$FAT_SMOKE_EXPECTED" == "0" ]]; then
    ...
    skipped_profile=$(log_count_pattern "INIT_FAT_SPAWN_SKIPPED")
    if [[ "$skipped_profile" -gt 0 ]]; then
      echo "[ok] FAT skipped marker present (INIT_FAT_SPAWN_SKIPPED count=${skipped_profile})"
    else
...
if [[ "$smoke_fail" -eq 1 ]]; then
  echo "[error] aarch64 optional-FS smoke: FAILED"
  if [[ -f "$LOGFILE" ]]; then
...
echo "[ok] aarch64 optional-FS smoke: all checks passed (status=$QEMU_STATUS)"
exit 0
