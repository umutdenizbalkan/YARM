...
if (( ! fail )); then
  ...
  for lit in "IPC_REPLY_TIMEOUT_OK arch=aarch64" "IPC_REPLY_BEATS_TIMEOUT_OK arch=aarch64" \
             "IPC_REPLY_TIMEOUT_ARMED arch=aarch64" "IPC_REPLY_TIMEOUT_LOCK_STATUS arch=aarch64" \
             "IPC_REPLY_TIMEOUT_LATE_SCAN arch=aarch64" "class=IpcReplyTimeout" \
             "IPC_REPLY_TIMEOUT_DEFERRED arch=aarch64"; do
...
if (( ! fail )); then
  ...
  forbid_log "$RW" \
    "IPC_REPLY_TIMEOUT_OK arch=aarch64 terminal=Timeout" \
    "scan_broad_lock=1" \
    "KERNEL PANIC" "RUST PANIC" "panicked at" "SYNCHRONOUS EXCEPTION" "Unhandled"
  recheck_sha_clean
  (( fail )) || RW_OK=1
