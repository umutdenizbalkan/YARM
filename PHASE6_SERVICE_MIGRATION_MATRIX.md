...
| Process Manager (`src/services/control_plane/process_manager/service.rs`) | budgeted timed receive (`ipc_recv_with_deadline`) in kernel roundtrip helper | request/response via kernel endpoint roundtrip helper | none | migrate request/reply helper to `IpcCall/IpcReply` where applicable | process | medium | 🟡 | PR-6.2, PR-6.3, PR-6.5 |
