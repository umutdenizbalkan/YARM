...
| Process Manager (`src/services/control_plane/process_manager/service.rs`) | typed in-memory request loop helper (not kernel receive loop) | request/response helper (`run_typed_request_loop`) | none | evaluate kernel IPC cutover needs; otherwise document non-kernel scope and waiver policy | process | low | 🟡 | PR-6.2, PR-6.5 |
