...
| Supervisor (`src/services/control_plane/supervisor/service.rs`) | budgeted helper (`recv_with_budget`) for control/fault queues | event-driven control handling; status-query reply now supports transferred reply-cap fallback path | receives fault/control events; no blocking legacy recv in loop | keep budgeted receive and migrate remaining request/reply-like ops to reply-cap path where safe | control-plane | medium | 🟡 | PR-6.3, PR-6.5 |
