...
#[test]
fn rpi5_hh3_register_boundary_is_stepwise_and_gates_hh4() {
    let boot = include_str!("../src/arch/aarch64/boot.rs");
    let start = boot
        .find("extern \"C\" fn yarm_rpi5_hh_rust_continue() -> !")
        .unwrap();
    let end = boot[start..].find("feature = \"rpi5-stage1\"").unwrap() + start;
    let continuation = &boot[start..end];

    let entry = continuation
        .find("rpi5_hh_write_line(&RPI5_HH_RUST_ENTRY_MARKER)")
        .unwrap();
    let after_entry = continuation
        .find("rpi5_hh_write_line(&RPI5_HH_RUST_AFTER_ENTRY_MARKER)")
        .unwrap();
    let pc_begin = continuation
        .find("rpi5_hh_write_line(&RPI5_HH_READ_PC_BEGIN_MARKER)")
        .unwrap();
    let pc_read = continuation.find("\"adr {pc}, .\"").unwrap();
    assert!(entry < after_entry);
    assert!(after_entry < pc_begin);
    assert!(pc_begin < pc_read);

    for marker in [
        "RPI5_HH_READ_PC_DONE_MARKER",
        "RPI5_HH_READ_SP_BEGIN_MARKER",
        "RPI5_HH_READ_SP_DONE_MARKER",
        "RPI5_HH_READ_VBAR_BEGIN_MARKER",
        "RPI5_HH_READ_VBAR_DONE_MARKER",
        "RPI5_HH_READ_TTBR_BEGIN_MARKER",
        "RPI5_HH_READ_TTBR_DONE_MARKER",
        "RPI5_HH_READ_TCR_BEGIN_MARKER",
        "RPI5_HH_READ_TCR_DONE_MARKER",
        "RPI5_HH_PRINT_REGS_BEGIN_MARKER",
        "RPI5_HH_PRINT_REGS_DONE_MARKER",
        "RPI5_HH3_PRECHECK_DONE_MARKER",
    ] {
        assert!(continuation.contains(marker), "missing {marker}");
    }
    assert!(boot.contains("struct Rpi5Hh3Ready;"));
    assert!(boot.contains("fn rpi5_hh4_retire_low_ttbr0(_hh3: Rpi5Hh3Ready)"));
    assert!(continuation.contains("let hh3 = Rpi5Hh3Ready;"));
    assert!(continuation.contains("rpi5_hh4_retire_low_ttbr0(hh3)"));
    assert!(boot.contains("RPI5_HH3_FAULT_BOUNDARY reason="));
    assert!(!continuation.contains("assert!"));
    assert!(!continuation.contains("panic!"));
}
