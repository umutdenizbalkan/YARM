...
- **PR-BND-6 pass B landed (stale-path doc cleanup):** boundary contract docs now describe the current multi-crate enforcement model (crate-graph primary gate + complementary source-shape guard), replacing stale single-crate-transition wording.
- **PR-BND-6 pass C landed (contract freeze lock):** boundary milestone docs now carry an explicit COMPLETE marker and freeze assertions are checked by `scripts/check-boundary-milestone-freeze.sh`.
