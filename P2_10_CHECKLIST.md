...
- [ ] Run strict QEMU smoke jobs for `x86_64`, `aarch64`, and `riscv64` on every pull request affecting `src/arch/**`, `src/kernel/vm.rs`, `src/kernel/frame_allocator.rs`, or boot/memory initialization paths.
- [ ] Keep non-strict smoke jobs as fast preflight, but require strict jobs to pass before merge.
- [ ] Publish strict smoke logs as workflow artifacts for every PR run.
...
- [ ] Add architecture-specific invalidation correctness tests that exercise page and ASID invalidation effects in non-hosted paths.
- [ ] Ensure each ISA test verifies required ordering/barrier semantics around invalidation.
- [ ] Record a per-ISA sign-off artifact (test log or report) in CI outputs.
...
- [ ] Add `P2_10_SIGNOFF_POLICY.md` describing required checks for page-table and allocator changes.
- [ ] Include explicit requirements for strict ISA smoke, invalidation correctness evidence, and allocator stress coverage.
- [ ] Add a PR template checklist section that references the sign-off policy.
...
- [ ] Add long-duration stress jobs focused on fragmentation, reserve/free churn, and map/unmap churn under ASID turnover.
- [ ] Define quantitative health metrics (e.g., allocation success ratio, max extent fragmentation, mean allocation latency proxy).
- [ ] Fail CI when metrics regress beyond agreed thresholds.
...
- [ ] Standardize telemetry counters/events for allocator pressure, ASID retire backlog, and invalidate operations.
- [ ] Add a concise troubleshooting runbook for common failure signatures.
