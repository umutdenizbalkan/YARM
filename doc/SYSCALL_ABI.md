...
| `24` | *(retired)* | — | — | `SpawnProcessFromUserBuf`, removed in U9-SPAWN2 §1. The number stays **unassigned and non-reusable**; see `RETIRED_SYSCALL_NUMBERS`. |
...
| `26` | *(retired)* | — | — | `SpawnFromInitramfsFile`, removed in U9-ASPACE1 §2. The number stays **unassigned and non-reusable**; see `RETIRED_SYSCALL_NUMBERS`. |
...
| `24` | *(retired)* | `SpawnProcessFromUserBuf`, removed in U9-SPAWN2 §1. PM's byte-copy spawn: it read a whole image through VFS into a user buffer and handed it to the kernel. Stage 197A had already made the MemoryObject zero-copy grant mandatory for every gated image; this was the last route contradicting that, and it survived only because the kernel image path table lacked the restart test's image 13. | Invoking `24` returns `InvalidNumber`, refused pre-lock by the split dispatcher. | — | — | `RETIRED_SYSCALL_NUMBERS` in `src/kernel/syscall.rs` |
| `26` | *(retired)* | `SpawnFromInitramfsFile`, removed in U9-ASPACE1 §2. **The entry this replaces was wrong**: it said PM used NR 26 "for image IDs `>= 4` through `pm_vfs_spawn_inline`", but that function calls `SpawnProcessFromUserBuf` (NR 24). NR 26's userspace wrapper had no caller anywhere, and no boot ever dispatched it. | Invoking `26` returns `InvalidNumber`, refused pre-lock by the split dispatcher rather than by the broad one. | — | — | `RETIRED_SYSCALL_NUMBERS` in `src/kernel/syscall.rs` |
