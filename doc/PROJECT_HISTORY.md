...
| **Direct IPC NR 6 / NR 7 — x86_64 production default ON (canonical 199D)** | **`IPC_DIRECT_PRODUCTION_QUIESCENT_SEAL nr6_ok=1 nr7_ok=1 census_ok=1 result=ok`**, exact commit `0b5ec254` — core boot `YARM_BOOT_OK`, all 6 service entries exactly once, `PM_ELF_ZC_FAIL count=0`, **53 NR6 + 41 NR7 ordinary syscalls off-lock with zero broad-lock entries**, waiter/lease bijection `result=ok` both directions; oracle regression `live_cells=2 result=ok`; ServerDies vector `[1;9]` with `created=54 closed=54 live_links=0`; both x86 reply-timeout matrix cells with zero `[fail]` lines. Zero fail/leak/duplicate/stale/fatal markers. The **first** live evidence of NR6/NR7 off-lock on the **production** path rather than under a proof knob. | **1** |
