...
> **real ACKs from both CPL0 and CPL3** (`qemu-x86_64-ap-saved-return-smoke`, three identical runs:
> `kernel_acks=2, user_acks=1`, `DONE result=ok context=user_origin`). Split unmap enforces
> **unmap before ACK before reclaim** with no domain lock held while waiting, targeting the
> VM-derived ASID residency mask (`live_cpu_bitmap_for_asid_split`), not an online/wake-only
> approximation. Ordinary `MemoryObject`/`DmaRegion` rollback and the functional D6 stack repair
> are now **broad-free**. Census **6 → 2**; the two survivors are the terminal broad dispatchers.
> Direct-IpcCall production remains **OFF**.
>
> ***U9 is NOT complete.*** It remains OPEN until those two terminal dispatchers are retired.
>
> A repeated-spin defect (a 20,000,000-iteration residency poll on every BSP trap) was
> INTRODUCED during this increment, BISECTED to it, FIXED and GUARDED — measured on
> `qemu-x86_64-ap-cross-cpu-request-smoke`: base 3/3 green, intermediate checkpoint 2/2 red,
> fixed 3/3 green.
>
> Six live profiles are red; **all six are exact-base deferrals with byte-identical failure
> signatures at base `d3032f8`, not candidate regressions** (both D6 core-smoke profiles,
> x86_64/AArch64/RISC-V server-dies, AArch64 exit-current-task, AArch64 shared-region-direct).
> **Neither D6 profile is claimed globally green** — only the §7 marker-family equivalence is.


Canonical census form is `with_cpu / with_broad / TOTAL` = **2 / 0 / 2**. The 3 raw
`self.state.lock()` sites are the BODIES of `SharedKernel::lock` / `with` / `with_cpu` — the
implementations every callsite goes through, not callsites themselves — so they are separate
bookkeeping and are never the third census field.

**U9-D3 took the census 6 → 2**, and the two survivors are exactly the terminal broad
dispatchers: the canonical Phase-2 trap dispatch in `src/arch/trap_entry.rs` and its RISC-V
counterpart in `src/arch/riscv64/trap.rs`. `src/runtime.rs` now holds **no production broad
acquisition of any kind**.

### 1.3 `with_cpu` — production callsites
...
| `src/arch/trap_entry.rs` | 2 | U3 retired the AArch64 FutexWait and Yield switch-success restores (11 → 9), then the two homologous x86_64 D2 switch-success restores — blocking send and blocking receive — onto one neutral exact-token transaction, `x86_post_lock_resume_marked_incoming` (9 → 7), then the x86_64 FutexWait and Yield switch-success restores, which reuse that same transaction unchanged (7 → 5), then the AArch64 `CurrentTaskExited` VALIDATION reacquisition onto the shared `post_lock_exit_validation_split` (5 → 4), then the AArch64 `CurrentTaskExited` REPLACEMENT RESTORE reacquisition onto `post_exit_replacement_restore_split` plus the off-lock `aarch64::trap::post_exit_restore_replacement` (4 → 3), then the AArch64 deferred-FutexWait NO-INCOMING idle current-TID read onto the EXISTING authoritative rank-1 transaction `SharedKernel::current_tid_authoritative(cpu)` — no new seam — which applies the same `validate_online_cpu` predicate `set_current_cpu` applies, performs the same `current_cpu` binding, and reads the same `current_tid_on(current_cpu())` (MPIDR-derived on freestanding AArch64), all under ONE rank-1 acquisition instead of the broad guard's two separate scheduler acquisitions (3 → 2). That read had previously been retired onto `current_tid_split_read` and RESTORED. `current_tid_authoritative` is the correct consumer — not the split reader, and not `terminal_idle_on_cpu_split`, whose extra `runnable_count_on` condition would strengthen the predicate. **Superseded — see the CPU-authority correction below.** That wording described `current_tid_authoritative` as validate-BIND-read. It no longer binds: `scheduler.current_cpu` is one process-global field, so an off-broad bind retargeted every ambient reader on every CPU. The helper is now validate-and-READ for the EXPLICIT CPU; its returned value is unchanged, and broad `with_cpu` keeps its own legacy binding. The predicate and the refusal policy are outcome-for-outcome unchanged: `Some(0)` is the idle task, and BOTH `no current task` and `CPU refused` arrive as `None` and are mapped to `true` by the same `unwrap_or(true)`. The TWO that remain are the canonical broad Phase-2 trap dispatch and the production post-switch architectural-state restore, whose ordinary x86_64/AArch64 path is split and lock-free, with a retained broad tail for functional D6 stack-repair cleanup — the latter's broad tail DELIBERATELY retained, because its body also runs the D6 proof cleanup (`d6_ensure_post_cleanup_task_stacks_mapped` maps kernel stack pages into the active root and every live task root), which is D3-fenced by `AI_AGENT_RULES` §14.4 and has no split form, so splitting the site would leave a broad drain behind at census delta 0. **U9/203C** moved the PRODUCTION post-switch architectural-state restore off the broad lock at `arch/trap_entry.rs`'s second acquisition. The site had been published as "the D3-fenced D6 controlled-proof restore"; both halves of that label were wrong. It was never D6-exclusive — `post_switch_restore_arch_thread_state` ran inside the same `with_cpu` on every ordinary single-CPU x86_64/AArch64 switch — and the `if is_proof_done` block it shared the acquisition with is not obsolete diagnostics: `D6_SWITCH_A_DONE` is emitted only there, for the D6-SWITCH-A *production* unlocked-switch milestone whose knob runs it with the controlled-proof knob OFF, and `d6_ensure_post_cleanup_task_stacks_mapped` is a functional cross-root stack repair without which a post-cleanup trap faults on a supervisor stack write (Stage 165D/165F/165G). The restore is now `post_switch_restore_arch_thread_state_split`: rank 1 (`post_switch_restore_admit_split` — the `validate_online_cpu` admission AND the `current_cpu` bind `with_cpu` performed, with the current TID read under that same guard, never through the deliberately non-binding `current_tid_split_read`) fully released, then ONE rank-2 acquisition (x86_64 reuses the EXISTING `owner_revalidation_snapshot_split`, re-keyed from an `OwnerRevalidationSelection` to the bare `tid` it always read out of it; AArch64 reuses the EXISTING `take_thread_restore_facts`) fully released, then the arch apply with NO lock held — `x86_apply_owner_revalidation_restore` on x86_64, carrying the complete pre-IRET CR3 invariant through `ensure_user_return_cr3_split`, and `apply_restored_thread_state` on AArch64, which now has one writer and three producers. The second premise this increment started from was also false: the production restore DOES perform cross-address-space page-table mutation, in `ensure_user_return_cr3`'s `hw_cr3 != task_cr3` repair branch — but it only MAPS, never unmaps, so it owes no shootdown, it reaches the page tables through a module that takes no `KernelState` lock, and U3 already shipped its production-live off-lock twin. RISC-V is not split: its stash gate opens with `!cfg!(target_arch = "riscv64")`, so the drain is statically unreachable there and it keeps its verbatim in-lock restore inside the retained tail. What still holds the lock is `post_switch_restore_broad_tail`, entered only by the D6 cleanup or that unreachable RISC-V arm; an ordinary production switch now enters no broad acquisition here. **The census does not move — `with_cpu` 6, `with_broad` 0, acquisitions 6 (census 6 / 0 / 6); the 3 raw `state_lock` method definitions in `runtime.rs` are separate bookkeeping and are NOT the third census field — and the reason it does not move is D3, not effort:** the cleanup allocates backing frames (`alloc_user_data_frame`, rank 6) and maps them across address spaces, so a split form would have to add `with_memory_split_mut`, fenced by `AI_AGENT_RULES` §14.4 until the lock-free `await_tlb_shootdown_ack` design and its multi-CPU smoke land — the same conclusion U3 recorded when it excluded this site. **U9-D3 (census 6 → 2).** The D3 fence that held this site — and the three ordinary rollback fallbacks in `runtime.rs` — is LIFTED, and the site is retired. Vector `0xF1` is now the SOLE target-side invalidation and generation-matched ACK producer: the trampoline's sched-idle mailbox service block is deleted, so no second producer can race the handler, and the handler reads its saved `CS` at a CPL-invariant offset to record `origin=kernel|user` alongside the ACK. It earns real ACKs from BOTH privilege levels, proven live and three times identically on `qemu-x86_64-ap-saved-return-smoke`: `kernel_acks=2, user_acks=1`, with `X86_TLB_SHOOTDOWN_ACK cpu=1 gen=3 origin=user` and `DONE result=ok context=user_origin targets=0x2 attempt=0`. `SharedKernel::complete_unmap_shootdown_split` drives it with NO domain or broad lock held — deliberately NOT `request_live_asid_shootdown`, which impersonates the requester onto each target, drains their mailboxes from the requester and calls `yield_current`. §6 then routed `MemoryObject`/`DmaRegion` through the phased composition, which now PERFORMS the active-transfer-mapping unmap (with real D3 completion), the supervisor `SUPERVISOR_OP_TRANSFER_REVOKED` report, the rank-6 `cap_refcount` drop and a reclaim gated on every target having acknowledged — unmap before ACK before reclaim, with a frame left OUT of the allocator rather than recycled when an ACK does not arrive. All three broad fallbacks went with it (6 → 3). §7 then split THIS tail: `d6_ensure_post_cleanup_task_stacks_mapped_split` takes rank 1 for the current TID, ONE rank-2 acquisition for every live task's kernel stack BY VALUE, then walks the page tables with nothing held, taking rank 6 for each frame ALLOCATION alone and releasing it before mapping — plus an exact rollback of any frame whose mapping failed, which the broad body leaked. Every D6 marker, counter and the `VmFull` verdict are unchanged, proven by exact-base comparison: across the whole 60-line D6 cleanup family in BOTH profiles (`D6_SWITCH_PROOF=1` and `D6_SWITCH_A=1`), base `d3032f8` and the split head differ in exactly one line — `D6_POST_CLEANUP_FIRST_TRAP_R14`, a kernel code pointer that moved because the code moved — with `STACK_MAP_DONE tasks=4 roots=3 failures=0 guard_pages=4`, `CLEANUP_DONE` and `D6_SWITCH_A_DONE` byte-identical. The RISC-V in-lock restore that shared the tail went with it and nothing reachable was removed: `maybe_switch_kernel_context` gates the stash on `!cfg!(target_arch = "riscv64")`, so the drain's `has_plan()` entry condition is never true there; all three arch arms of the FOUNDATION `post_switch_restore_arch_thread_state` remain defined (3 → 2). **The two survivors are the terminal broad dispatchers, out of scope for every pass.** |
...
| `src/arch/x86_64/smp.rs` | 0 | U3 retired the AP saved-resume enqueue→dispatch placement onto `enqueue_then_dispatch_on_cpu_split` (4 → 3), the AP return-to-idle `block_current_on_cpu` onto `block_current_on_cpu_split` (3 → 2), the BSP saved-resume preempt-prefer onto `on_preempt_prefer_on_cpu_split` (2 → 1), and finally the ED-2 next-task placement onto the same `enqueue_then_dispatch_on_cpu_split` under `EnqueueRefusalPolicy::Decline` (1 → **0**), once the existing SELECTOR-OFF two-task AP workload was proven to reach it live. **This file is now fully drained.** |
| `src/arch/riscv64/boot.rs` | 1 | 1048 |

Structural reading of those 7, re-derived from the final tree (full per-site inventory in
§1.4a):

* **2 are the authoritative trap dispatch** — `trap_entry.rs:310`
  (`handle_trap_entry_shared`, shared by x86_64 and AArch64) and its RISC-V twin
  `riscv64/trap.rs:829` (`handle_riscv_trap_entry_shared`). These are *the* global lock of the
  system: every syscall that is not on the split whitelist, plus every timer IRQ, external IRQ
  and page fault, runs its entire handler inside this closure.
* **0 are post-lock re-acquisitions.** The whole D2/D6/FutexWait/Yield/exit drain class is
  retired onto narrow seams. `riscv64/trap.rs` and `arch/x86_64/smp.rs`'s saved-resume paths
  hold none; `trap_entry.rs` keeps only the D3-fenced D6 controlled-**proof** restore at
  `trap_entry.rs:1694`, which is not a plain restore drain — its body also performs
  cross-address-space page-table mutation.
* **`src/arch/x86_64/smp.rs` contributes 0.** Its last acquisition, the ED-2 next-task
  placement in `ap_sched_next_or_idle`, was retired once the existing selector-off two-task AP
  workload was proven to reach it live. That file is now fully drained.
* **4 are the recv / delivery boundary in `runtime.rs`** — one Phase-A multi-domain composite
  (`try_split_ipc_recv_queued_plain_into_frame:4093`) and three re-entries of the same
  `rollback_materialized_recv_cap` capability-teardown body (`4193`, `4486`, `5278`).
* **The identity, SMP, deadline, thread-creation and RISC-V-resume groups are gone.**
  `descriptor_tables.rs`, `thread_state.rs` and `riscv64/boot.rs` each hold **0** broad
  acquisitions today.
...
### 1.4 Broad `.with(|state| …)` — 0 production callsites

There is **no** production broad `SharedKernel::with` callsite left anywhere in the tree.
The last one — the `ap_saved_resume_context` read in `c2c_bsp_saved_frame_resume`
(`src/arch/x86_64/smp.rs:2453`) — was retired by U3 once its path became live-reached; see
the closing subsection below.
...
**The BSP read at 2453 was initially NOT converted, and has since been retired.** Baseline
reachability at `f7e25f2` showed `c2c_bsp_saved_frame_resume` was never live-reached: across
five runs of `qemu-x86_64-ap-cross-cpu-reply-smoke.sh` the chain died before the reply was
delivered, so `X86_BSP_SAVED_DISPATCH_OK cpu=0 mode=saved` never fired (0/5). An unreached
site is not retired merely because it is homologous to a reached one, so it kept the broad
read while that held.

**U3 then retired BOTH BSP saved-resume acquisitions in one pass (1 → 0 broad, 9 → 8
`with_cpu`).** The explicit-CPU authority and the deterministic REQUEST_OK proof rendezvous
delivered at `c2b9b77` made the path repeatedly live-reached, and the retirement was proven
against it: five consecutive clean runs of the same script, twelve named markers each exactly
once per run, `X86_BSP_SAVED_DISPATCH_OK cpu=0 mode=saved` among them (5/5).

* The **saved-context read** moved onto the EXISTING rank-2 snapshot
  `SharedKernel::ap_saved_resume_context_split` — the same one the AP site runs, so no second
  reader was created — destructured by NAME (`ApSavedResumeContext { asid, cr3, rip, rsp,
  gprs, fs_base, runnable_saved }`). The retired positional 7-tuple could not prevent a
  consumer transposing `rip`/`rsp` or reading `fs_base` where `cr3` was meant. Refusals are
  unchanged outcome for outcome, as tabulated for the AP conversion above.
* The **preempt-prefer mutation** moved onto ONE new rank-1 transaction,
  ...
  wrapper ran — that wrapper immediately re-entered the rank-1 scheduler lock anyway, so the
  broad guard added nothing but width. The policy still has exactly one implementation.
  **The legacy fallback is preserved deliberately, not improved:** when the preferred TID is
  not queued on that CPU the scheduler may still select some other runnable task and return
  it, and the caller's `made_current != Some(client_tid)` comparison is what rejects that. An
  invalid or offline CPU still yields `None` with no mutation — reached now by the scheduler's
  own `check_online_cpu` rather than by `with_cpu` refusing and `.unwrap_or(None)` collapsing
  the error.

Neither transaction reads or writes the process-global ambient `scheduler.current_cpu`: both
are named by the explicit `cpu` the caller trapped on. Identity is **not** strengthened — this
path holds only a numeric `client_tid` and the ASID the snapshot discovers, carries no
generation-bearing incarnation token, and no `DispatchMarkToken` or generation authority was
fabricated for it. The consumer's 8-step ordering is intact, snapshot still BEFORE the
scheduler mutation, and every domain guard is released before `DONE.swap`, the marker,
`configure_syscall_msrs_for_self`, the FS-base WRMSR, the CR3 write, the frame construction and
`resume_user_mode_iret`. Both caller-free `KernelState` helpers
(`ap_saved_resume_context`, `on_preempt_prefer_on_cpu`) were deleted.
...
`KernelState::handle_trap`'s BROAD syscall and timer arms — so it ran only on traps the split
routes declined. That was never a designed dependency; it held because some syscall class always
fell through. On the x86_64 SMP oracle profile the 114 blocking NR 2 receives per boot that used
to yield now commit a queue advance pre-lock, the audit stopped being reached, and CPU 1 never
received a workload at all.

It is bounded to ONE acquisition per boot by off-lock pre-screening of every gate the audit
itself applies — its own one-shot latch read lock-free, `present > 1` through the topology split
read, a real user task current through the authoritative split read, and the graduated proof's
completion flag. An `-smp 1` boot never acquires. It is classified **boot-only**, not
runtime-required: no steady-state syscall, trap or drain depends on it. And it lives in
`trap_entry.rs` rather than on `SharedKernel` so that `src/runtime.rs` keeps the zero this table
publishes.
...
| Phase-A dependency | Domain / rank | Off-lock form already in the tree |
|---|---|---|
| `is_kernel_task` (`current_task_has_user_asid`) | scheduler 1 → task 2 | `current_tid_authoritative(cpu)` (already called *before* the closure) + `task_asid_for_tid_split_read` |
| `RecvRequest` construction, `plan_recv_core` | none (frame only) | pure |
| `resolve_endpoint_index` | config + IPC 3 | `runtime_capacity_config_split_read` + `with_ipc_split_mut` |
| `ipc_try_recv_queued_*` dequeue + refill | IPC 3 | body is already `with_ipc_state_mut`-only; `with_ipc_split_mut` hosts it unchanged |
| `note_endpoint_only_queued_recv_split` | telemetry 10 | IPC-domain counter, same acquisition |
| ordinary-cap envelope snapshot | IPC 3 + capability 4 + task 2 | consume/resolve/cnode reads, each already split-available; the **mint** is already off-lock (`complete_recv_boundary_ordinary_cap`, 186D2/186D3) |
| deferred sender wake | scheduler 1 | `apply_split_sender_wake_plan_split(cpu, …)` — already exists and is already used off-lock |
| user writeback | — | already deferred to Phase B (186E seam) |
| **reply-cap materialization** | **capability 4 mint + IPC 3 reply-record bind** | **none.** `materialize_received_message_cap_routed_with_delegation_split` explicitly refuses this class, returning `CapTransferMaterializeOutcome::DeferredReplyCap` |

The reply-cap arm is not a dormant edge case. On an **ordinary default x86_64 core smoke** — no
oracle, no selector — Phase A performs exactly one reply-cap materialization:
...
`IPC_RECV_BOUNDARY_SNAPSHOT_OK receiver_tid=3 cap=65538 reply=true`. It is PM receiving a call
that carries a reply cap, and it happens on every boot.

That rules out the two otherwise-available exits. Refusing the class **pre-dequeue** would not be
an "exact refusal of a non-admitted case" — the class is admitted and live-served today, so the
refusal would reroute a default production path onto the legacy global dispatcher, i.e. onto
the very terminal-dispatcher acquisition U9 exists to delete. And refusing it **post-dequeue** is
impossible: the message has already been consumed by then, so there is nothing to fall back to.

Retiring row 4 therefore requires minting a receiver-local reply cap (capability, rank 4) while
the reply-object record is bound in the IPC domain (rank 3) — the unsolved
`reply_cap_ipc_rank_inversion`. It is a capability-domain fence, and U9's own stop rule applies.
...
**The capability-rollback cohort (rows 5–7).** All three call the single body
`KernelState::rollback_materialized_recv_cap`, on three different failure paths. They are
dependency-blocked, not merely unconverted: unwinding a materialized receive capability
touches the delegation graph and notification teardown, and the reclaim path is subject to the
same D3 shootdown fence as row 3. No split form exists, and inventing one is out of scope for a
callsite-by-callsite retirement.

U9 re-verified this against source rather than carrying the description forward. The body is
object-authoritative (`src/kernel/boot/transfer_state.rs:193`), and its Reply arm runs
`fast_revoke_reply_cap_in_cnode` followed by `clear_reply_cap_waiter_cap` — a **capability-domain
rollback that must also mutate the IPC reply record**. That is the same
`reply_cap_ipc_rank_inversion` as row 4, taken in the opposite direction. The transfer arm runs
`revoke_capability_in_cnode`, which reaches object reclaim. Both arms therefore stay
CAP/VM-teardown fenced; the §1 exclusion holds unchanged.

**Groups that no longer exist.** The earlier enumeration listed *post-lock drain
re-acquisitions* (14 sites across `trap_entry.rs` and `riscv64/trap.rs`), a three-site *first
resume trampoline* group, an *AArch64 split return path*, a *RISC-V resume* site in
`riscv64/boot.rs`, and an *identity / SMP / deadline helpers* group in `runtime.rs`. Every one
of those has been retired onto a narrow seam by Stage 199D and directives U1–U3; none appears
in the current tree. `src/kernel/boot/thread_state.rs`, `src/arch/riscv64/boot.rs`,
`src/arch/x86_64/descriptor_tables.rs` and now `src/arch/x86_64/smp.rs` hold **zero**
acquisitions each.

> **The earlier naming hazard is resolved.** A previous revision of this section warned that
> `smp_request_wake_target_split_read` was named `*_split_read` while taking the broad lock via
> `self.with(|k| k.task_home_cpu(tid))`. U3 retired that acquisition: the function now reads
> `cpu_affinity` through `with_task_tcbs_split_mut`, so its name and its lock behaviour agree.
> No `*_split*`-named function in the tree takes a broad lock today.

> **Historical snapshot, retained as provenance only.** The counts quoted in older paragraphs of
> this document — 51, 50, 49, 47, 44, 39, 21 runtime-required, 8, and the per-group line numbers
> that went with them — are historical and are **not** claims about the current tree. The
> authority for the current census is `tests/broad_lock_census_guard.rs`, which recomputes it
...
| `FutexWait` NR 9 | AArch64 | in-lock + Stage 195E drain (`trap_entry.rs:612`) | **broad** + drain | blocks caller | 195E drain | as above | suite | `FIRST_COHORT_SEAL arch=aarch64 class=FutexWait result=ok proof=live` — see the idle-oracle note below |
| `FutexWait` NR 9 | RISC-V | in-lock, typed idle outcome | **broad** | blocks caller | typed `EnterKernelIdle` | typed outcome, not an `Err` sentinel | suite | `FIRST_COHORT_SEAL arch=riscv64 class=FutexWait result=ok proof=live`; `RISCV_FUTEX_WAIT_IDLE_ORACLE_DONE result=ok lock_dropped=1 current_none=1 outgoing_blocked=1` when the idle oracle completes — see the note below |
...
| `IpcRecvTimeout` NR 5 | x86_64 | deadline pre-read split; completion via `OffLockReplyTimeout` | **broad** entry + **split** completion | blocks receiver until deadline | `run_due_ipc_timeout_work` → `drain_reply_timeout_post_work` (U7: ungated production) | terminal-claim + generation-bearing deadline token; reply-wins disarms | suite | `IPC_REPLY_TIMEOUT_LOCK_STATUS arch=x86_64 scan_broad_lock=0 completion_transaction_narrow=1 classes=IpcReplyTimeout+IpcSendTimeout production=1 result=ok` (ordinary boot) |
| `IpcRecvTimeout` NR 5 | AArch64 | port landed 200C2C1B; U7 promoted the scan to production | **split** scan + **split** completion | as above | shared `run_due_ipc_timeout_work` | as above | suite | matrix cell + `IPC_REPLY_TIMEOUT_LOCK_STATUS arch=aarch64 scan_broad_lock=0 … production=1` (ordinary boot) |
| `IpcRecvTimeout` NR 5 | RISC-V | port landed 200C2C2C-R2A/B; U7 promoted the scan to production | **split** scan + **split** completion | as above | RISC-V Phase-3 `run_due_ipc_timeout_work` | causal reply-win reservation | suite | matrix cell + `IPC_REPLY_TIMEOUT_LOCK_STATUS arch=riscv64 scan_broad_lock=0 … production=1` (ordinary boot) |
| `IpcSendTimeout` (U7, 199E) | all 3 | `collect_due_ipc_timeout_work` + `drain_send_timeout_post_work` | **split** scan + **split** settle | wakes a timed-out blocking sender | `run_due_ipc_timeout_work` (ungated, one driver per port) | exact `{tid, asid, send_generation}` claim; U6 publish-before-wake; one envelope consume + one no-reclaim pin release | suite (`u7_production_timeout_promotion`) | `U7_SEND_TIMEOUT_SETTLED … broad_lock=0 result=ok`; seal `GLOBAL_LOCK_RETIRE_CLASS_DONE class=IpcSendTimeout` from the U6 delivery point |
...
> **Idle-oracle note (U3 / 203C, AArch64 FutexWait no-incoming idle).** The `…_IDLE_ORACLE_DONE`
> markers are the evidence for the *idle outcome* of the FutexWait drains, and they are separate
> from the FutexWait **class** seal, which is live-proven on all three architectures. On AArch64
> the idle-oracle workload does **not** complete: a fresh `qemu-first-cohort-retirement-seal.sh`
> run emits `AARCH64_FUTEX_WAIT_IDLE_ORACLE_PROVISION_OK` = 1 and
> `AARCH64_FUTEX_WAIT_IDLE_ORACLE_SET` = 1 but `AARCH64_FUTEX_WAIT_IDLE_ORACLE_DONE` = **0**, and
> the seal reports `aarch64 FutexWait idle oracle proof missing`. Its precondition ("all servers up
> and blocked on recv") is unreachable behind the pre-existing SpawnV5/initramfs stall. That is
> true at the U3 AArch64 FutexWait-idle checkpoint **and** at its exact base `6dd3ca4`, with an
> identical marker census at both, so the retirement of that read is an **exact-base deferral for
> the target AArch64 cell** and is **not** live proof of it. The RISC-V idle oracle is
> **intermittent** under the harness's timing bound — observed absent at `6dd3ca4` and present at
> the checkpoint from bit-identical RISC-V kernel binaries — so its `…_DONE` marker is not a
> stable gate either. The rows above previously named the AArch64 `…_DONE` marker as that cell's
> live evidence; that claim is corrected here rather than carried forward.
...
|-------|--------------------|--------|------------------|
| **199C** | Blocking `IpcSend` — sender-waiter publication retired, sparse-queue + timeout parity | **DELIVERED / CLOSED** | The publication route is off-lock: `blocking_send_publication_snapshot` mutates nothing in-lock and stashes the **complete canonical `Message`**; `commit_blocking_send_split` publishes the waiter in a rank 1 → 2 → 3 transaction. U6-FRAME live-proved the exit condition on **x86_64 and RISC-V** (three consecutive clean runs each): `U6_BLOCKING_SEND_INLOCK=0`, waiter publication once, completion `result=0` once, zero stale refusals, `IPC_RECV_V2_SENDER_WAKE_ORDER_OK` once, and the receiver observing the blocked sender's exact envelope (`sender_tid=10008 opcode=0 len=10`) once at the FIFO tail. Sparse-queue behaviour unchanged; timeout parity carried by the closed 199E pipeline. AArch64 keeps the strict exact-base pre-admission deferral at `INIT_SPAWN_V5_CALL_BEGIN` — no live U6 proof claimed there. |
...
| **199E** | IPC timeout + cancellation: recv, **send**, **call**, reply | **DELIVERED / CLOSED** | U7 delivered this stage; the row is corrected here from source, having been carried forward stale. **The off-lock scan is NOT x86_64-only:** the production entry `SharedKernel::run_due_ipc_timeout_work` carries no `cfg(target_arch)` and is wired on all three architectures — `src/arch/trap_entry.rs:1759` (x86_64 + AArch64) and `src/arch/riscv64/trap.rs:931`. **`IpcSend` timeout IS retired:** `run_due_ipc_timeout_work_at` runs one scan (`collect_due_ipc_timeout_work`) then the class drains — `drain_reply_timeout_post_work` and `drain_send_timeout_post_work` — in that fixed order, off the broad lock. `IpcCall`'s timeout is the reply-wait deadline and is covered by the reply class; no separate call-timeout machinery exists. The cited broad fallback at `runtime.rs:3725` no longer exists — that line is now inside an unrelated transaction, and the obsolete `SharedKernel::run_reply_timeout_completion` wrapper was deleted by U1. The single completion body `KernelState::run_reply_timeout_completion_locked` is unchanged and is still reached directly by the in-lock timer scan, which is not a fallback. Live: `STAGE_200_IPC_REPLY_TIMEOUT_MATRIX_SEAL` (`72a4ebf`). |
...
| **203C** | Scheduler core; rank-1/rank-2 seams become **authoritative, not compatibility helpers** | **OPEN** — partial | **Correction, from source.** This row previously read "the rank-1 seam is authoritative for queue-advancing dispatch on **x86_64 only**", citing `d6_genuine_enabled()`. That cites the wrong predicate: U4 introduced `queue_advancing_dispatch_enabled()` (`src/kernel/boot/mod.rs`) as the CANONICAL predicate for queue-advancing blocking dispatch, and it is **architecture-neutral** — `!d6_controlled_switch_proof_enabled() && !d6_switch_a_enabled()`, with no `cfg!(target_arch)`. `d6_genuine_enabled()` still exists and still is `cfg!(target_arch = "x86_64") && …`, but it governs the D6 genuine-seam observation, a different question its own doc comment spells out. Queue-advancing dispatch is therefore not x86_64-only. The drain re-acquisition class is fully retired: of the **7** remaining runtime-required callsites, **0** are post-lock drains (§1.4a). What remains is the authoritative trap dispatch itself (2), the production post-switch architectural-state restore (ordinary x86_64/AArch64 path split and lock-free; retained broad tail for functional D6 stack-repair cleanup) (1) and the recv/delivery boundary (4). **U9/203C label correction:** that middle site was published as "the D3-fenced D6 controlled-proof restore". It was never D6-exclusive — the PRODUCTION post-switch architectural-state restore ran inside the same acquisition — and it is no longer a restore: U9 split that restore onto `post_switch_restore_arch_thread_state_split` (rank-1 admission and bind, rank-2 gather, then a lock-free arch apply), so an ordinary production switch on x86_64 or AArch64 now enters no broad acquisition here. What is left is `post_switch_restore_broad_tail`: the D6 proof / D6-SWITCH-A cleanup, plus the statically-unreachable RISC-V restore. The census does not move, and the reason is D3, not effort — `d6_ensure_post_cleanup_task_stacks_mapped` allocates backing frames (rank 6) and maps them across address spaces, so a split form would need `with_memory_split_mut`, fenced by §14.4. **203C stays OPEN/PARTIAL** because scheduler operations still execute inside those two authoritative trap dispatches and that site's D3-fenced broad tail remains. |
...
| Stage | Scope | Status | Evidence and gap |
|-------|-------|--------|------------------|
| **204A** | Broad-lock callsite census, every runtime use classified boot-only / test-only / runtime-required / obsolete fallback; **no undocumented runtime callsite** | **COMPLETE** | §1.4a: all **7** callsites enumerated with file, line, enclosing symbol, form, role, why it remains and expected retiring directive. **0 boot-only, 0 test-only, 0 obsolete, 7 runtime-required, 0 undocumented.** (Stage 199D retired the AArch64 handled-split syscall return; U1 deleted the two obsolete acquisitions, 49 → 47; U2 relocated the three test-only ones, 47 → 44; U3 has since retired the entire drain class, both x86_64 BSP saved-resume acquisitions and the x86_64 ED-2 next-task placement, 44 → … → 10 → 8 → **7**. The census is exactly the runtime-required set.) Raw/global `KernelState` mutation outside the three `SharedKernel` methods: none exists (§1.5). Kept honest by `tests/broad_lock_census_guard.rs` (7 tests), which recomputes the census from source and fails on any added or removed production callsite. |
...
is now one rank-1 scheduler transaction that validates the EXPLICIT CPU and reads it without binding `current_cpu` (see the CPU-authority note in §1.3)
...
| `complete_blocked_waiter_delivery_split`, reached from `execute_dispatch_post_work` and `execute_blocked_waiter_{reply,ordinary}_cap_delivery` | post-lock capability delivery |
