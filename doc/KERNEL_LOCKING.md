...
| runtime-required | **39** | the real retirement surface; U3 has retired five RISC-V post-lock drains — two read-only current-TID reads onto the rank-1 scheduler seam, and the three switch/restore drains onto the exact-token rank-2 transaction |
