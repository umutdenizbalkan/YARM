...
| runtime-required | **36** | the real retirement surface; U3 has retired eight post-lock drains — six on RISC-V (five switch/read drains plus the `CurrentTaskExited` validation snapshot, onto a coherent rank-1/rank-2 transaction) and two on AArch64 (the FutexWait and Yield switch-success restores, onto the neutral exact-token resume core) |
