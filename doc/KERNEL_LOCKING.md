...
`self.state.lock()` sites are the BODIES of `SharedKernel::lock` / `with` / `with_cpu`, not
callsites, so they are separate bookkeeping and never the third census field.
...
counterpart in `src/arch/riscv64/trap.rs`. Every other file, `src/runtime.rs` included, now
holds zero.
...
| runtime-required | **0** | the real retirement surface; U3 has retired twelve post-lock drains — six on RISC-V (five switch/read drains plus the `CurrentTaskExited` validation snapshot, onto a coherent rank-1/rank-2 transaction), two on AArch64 (the FutexWait and Yield switch-success restores, onto the neutral exact-token resume core) and four on x86_64 (the D2 blocking-send/blocking-receive and the FutexWait/Yield switch-success restores, all onto one neutral exact-token transaction) — plus the three current-identity acquisitions, all four broad `with` wrappers in `runtime.rs` (onto the rank-2 task seam and a sequential rank-2 → rank-3 disarm), and the x86_64 AP saved-context read (onto the rank-2 `ap_saved_resume_context_split` snapshot). The BSP saved-context read was retained at that point because its path was not yet live-reached; it has since been retired too (see the closing paragraph of this row). U3 then retired the AP saved-resume enqueue→dispatch placement onto `enqueue_then_dispatch_on_cpu_split`, one rank-1 → rank-2 transaction (validate → bind `current_cpu` → nested rank-2 enqueue-policy resolution → `enqueue_on_with_priority` → `dispatch_next_on`), so `ap_saved_frame_resume` holds no broad lock at all. The homologous next-task placement in `ap_sched_next_or_idle` is NOT converted: every script that sets `yarm.ap_user_dispatch=1` also sets `yarm.ipccall_direct_smp_oracle=1`, which pins `ap_workload_task_count()` to 1, so `AP_DISPATCH_COUNT < count` is false after the first ring-3 entry and its unique marker `X86_AP_NEXT_TASK_DISPATCH_BEGIN` never fires. It keeps its acquisition byte-for-byte, including its distinct `Err(_) => None` refusal policy. U3 finally retired the AP return-to-idle `block_current_on_cpu` onto `block_current_on_cpu_split`, a rank-1-only scheduler transaction (validate → bind `current_cpu` → the existing `block_current_on` primitive) that takes no rank-2 lock and changes no `TaskStatus`, so `ap_seal_return_to_idle` holds no broad lock either. U3 finally retired the x86_64 D6 first-resume CPU binding onto `bind_current_cpu_split`, a rank-1-only scheduler transaction (validate → bind `current_cpu`, nothing else): its broad body's single call, `post_switch_restore_arch_thread_state(kernel, cpu, None)`, returns `Ok(())` on the first statement of the x86_64 `restore_arch_thread_state` when `frame` is `None` — before any current-TID read, TCB access, context/TLS restore, ASID activation, CR3 check or domain lock — so the acquisition's only `KernelState` effect was the CPU validate-and-bind `with_cpu` performs on entry. `thread_state.rs` is now fully drained; the REAL D6 restore/cleanup acquisition in `trap_entry.rs` passes a live frame and is untouched. U3 finally retired the three homologous blocked-waiter Phase-C completions in `runtime.rs` — `execute_dispatch_post_work`, `execute_blocked_waiter_reply_cap_delivery` and `execute_blocked_waiter_ordinary_cap_delivery`, which each re-entered the broad lock with a byte-identical body — onto ONE shared, class-neutral, rank-ordered transaction, `complete_blocked_waiter_delivery_split`: rank 1 (`validate_online_cpu` + bind `current_cpu`, exactly what `with_cpu` did on entry), rank 2 (clear the waiter's blocked-recv return registers, then read the wake TID's current ASID), rank 3 (the identity-keyed `clear_endpoint_waiter_if_identity`, so last-receiver-wins is unchanged), then the wake as rank 2 → rank 1. Every seam is entered at a strictly higher rank than the one held, so rank 1 is never taken under rank 2 or 3. The three call sites keep their distinct pre-transaction snapshots and their result stays ignored exactly as the broad `let _ = self.with_cpu(...)` ignored it. U3 finally retired the x86_64 post-drain idle-owner revalidation, `revalidate_idle_owner_after_drains`, onto a CPU-local transaction: rank 1 (`validate_online_cpu` — the same predicate `set_current_cpu` applies — then bind `current_cpu`, then the SAME single `dispatch_next_on(cpu)` advance, carried out as a typed `OwnerRevalidationSelection` rather than a bare TID), rank 1 released, then ONE rank-2 acquisition fusing the restorability verdict with the restore payload (`user_context`, the pending TLS-restore request taken exactly once, and the task's OPTIONAL `asid`), rank 2 released, then the frame / FS-base / `LAST_RESTORED_TLS_BASE` / pre-IRET CR3 work with NO lock held (through the existing `ensure_user_return_cr3_split`, repair path and fail-safe skip included). Rollback re-takes rank 1 alone. No `TaskStatus` is written, no ASID activation is added, and no `DispatchMarkToken` is minted — the retired body did none of those. U3 finally retired the two homologous recv copy-fault completions in `complete_recv_boundary_user_copy` — the plain `CopyFault` arm and the recv-v2 `PayloadCopyFault` arm, whose bodies were byte-identical — onto ONE class-neutral transaction, `record_recv_boundary_user_fault_split`: rank 1 (the existing `bind_current_cpu_split`, promoted from its x86_64 gate to architecture-neutral because nothing in it was architecture-specific) fully released, then rank 8 (the existing `record_fault_split_mut`) fully released, then `frame.set_err(PageFault)` with no lock held — record before frame error, exactly as `record_user_fault` ordered them. A refused CPU records nothing and leaves the frame untouched, matching the broad closure that never ran. No new seam was added. The capability-rollback re-entry in the same method is dependency-blocked (D3 shootdown, delegation graph, notification teardown) and its broad acquisition and body remain byte-for-byte intact. **U9-F (CENSUS-DELTA 0)** established that only two of the six obligations `revoke_capability_in_cnode` owes actually depend on D3 — the active-transfer-mapping unmap + TLB shootdown, and the memory-object refcount drop + frame reclaim — and moved the other four (cross-CNode descendant revocation, delegation-link removal, the recursive in-cspace revoke, and notification destroy + waiter wake) into a phased composition in `capability_lifecycle_state.rs`, the existing owner of the broad function, with no seam file and no marker family added. All three rollback callsites now OFFER that split first and fall back to this unchanged broad acquisition on refusal, so no acquisition is retired and the census is unmoved at 6; `Reply` stays with U9-C and `MemoryObject`/`DmaRegion` stay D3-fenced. U3 finally retired the RISC-V post-lock blocked-syscall TERMINAL-IDLE predicate onto `terminal_idle_on_cpu_split`, ONE coherent rank-1 scheduler snapshot: validate with `validate_online_cpu`, bind `current_cpu` exactly as `with_cpu` did, then read `current_tid_on(cpu)` and `runnable_count_on(cpu)` under that same single guard and return the identical `matches!(current, None | Some(0)) && runnable == 0`. The broad body's two reads each took the scheduler lock separately and were made coherent only by the broad guard; fusing them is why this is not `current_tid_authoritative` plus a second count read. A refused CPU returns the same error class, mutates nothing, and the caller's `.unwrap_or(false)` reports the same `false`. **`src/arch/riscv64/trap.rs` now holds no reacquisition at all** — its one remaining production `with_cpu` is the canonical broad Phase-2 trap handler. U3 finally retired the ordinary-cap DEFERRED SENDER WAKE in `complete_recv_boundary_ordinary_cap` onto `apply_split_sender_wake_plan_split`: rank 1 (`bind_current_cpu_split` — the same `validate_online_cpu` predicate `set_current_cpu` applies) released, then the `IPC_RECV_SPLIT_REFILL_WAKE_APPLY` marker with identical text, then the SHARED `wake_tid_to_runnable_split` (rank 2 status/timeout/placement snapshot released, then rank 1 enqueue). No wake logic is duplicated: that body already served the blocked-waiter completion and was renamed from `wake_blocked_waiter_split` now that it has a second production caller — one writer, two callers. A CPU refusal emits no marker and wakes nothing, exactly as the broad closure that never ran. Deliberately not routed through `complete_blocked_waiter_delivery_split`, which additionally clears receiver return registers and endpoint-waiter state that sender wake must not touch. U3 finally retired BOTH AArch64 `CurrentTaskExited` post-lock reacquisitions: the VALIDATION snapshot onto the shared `post_lock_exit_validation_split` the RISC-V consumer already used, and the REPLACEMENT RESTORE onto `post_exit_replacement_restore_split` — rank 1 validates the CPU with the same `validate_online_cpu` predicate and binds `current_cpu`, rank 2 nested inside it gathers the exact replacement's ASID, saved context, TLS request and parked blocked-syscall completion in ONE acquisition, then every lock is released before `aarch64::trap::post_exit_restore_replacement` performs the TTBR0_EL1 activation and the frame restoration through the SAME single frame writer the in-lock restore uses (`apply_restored_thread_state`), so completion-before-argument-mirror ordering and the AArch64 error-lane convention stay single-sourced. Replacement TID reuse under a different ASID, a replacement that is no longer this CPU's current task, a missing ASID or context, and reselection of the exiting task all fail closed. The AArch64 exit consumer now holds no broad acquisition at all. The D6 controlled-switch proof restore in `trap_entry.rs` is NOT part of that cohort and stays broad: its body also performs the D6 proof cleanup, whose `d6_ensure_post_cleanup_task_stacks_mapped` maps kernel stack pages into the active root and every live task root — cross-address-space page-table mutation to which the `AI_AGENT_RULES` §14.4 D3 fence applies, with no split form in existence. U3 finally retired the AArch64 deferred-FutexWait NO-INCOMING IDLE current-TID read — `with_cpu(cpu, |kernel| matches!(kernel.current_tid(), None | Some(0))).unwrap_or(true)`, a single confirmation that `current` is absent or idle before diverging into the BSP idle loop — onto the EXISTING authoritative rank-1 transaction `current_tid_authoritative(cpu)`, creating NO new seam. That helper applies the same `validate_online_cpu` predicate `set_current_cpu` applies and reads the current TID under ONE rank-1 acquisition, where the broad body took the scheduler lock twice and was coherent only by virtue of the broad guard. **CPU-authority correction (canonical 203C saved-resume prerequisite):** the helper is validate-and-READ for the EXPLICIT CPU — it does NOT bind `scheduler.current_cpu`, and it does not resolve the lookup through any ambient selector. Binding from an off-broad seam retargeted every ambient reader on every CPU, including a CPU mid-syscall inside the broad lock. The returned value is unchanged for every caller (on freestanding AArch64 the retired `MPIDR_EL1` lookup is value-identical, because the AArch64 trap entry derives the `cpu` it passes by that same expression), and broad `with_cpu` retains its own legacy binding. The predicate and the refusal policy are unchanged outcome for outcome: `Some(0)` is the idle task, and BOTH `no current task` and `CPU refused` arrive as `None` and are mapped to `true` by the same `unwrap_or(true)`. `current_tid_split_read` is deliberately NOT used — it does not bind `current_cpu`, which is why the earlier substitution at this callsite was reverted — and neither is `terminal_idle_on_cpu_split`, whose extra `runnable_count_on` condition would strengthen the predicate. **`src/arch/trap_entry.rs` now holds exactly two acquisitions**: the canonical broad Phase-2 trap dispatch and the production post-switch architectural-state restore, whose ordinary x86_64/AArch64 path is split and lock-free, with a retained broad tail for functional D6 stack-repair cleanup. U3 finally retired BOTH x86_64 BSP saved-resume acquisitions in `c2c_bsp_saved_frame_resume`, in one pass, once the cross-CPU reply workload began reaching that path. The SAVED-CONTEXT read — `shared.with(|k| k.ap_saved_resume_context(client_tid))`, the LAST production broad `SharedKernel::with` in the tree — moved onto the EXISTING rank-2 snapshot `ap_saved_resume_context_split` that the AP site already ran, so no second reader was created; it is destructured by NAME (`ApSavedResumeContext { asid, cr3, rip, rsp, gprs, fs_base, runnable_saved }`), which the retired positional 7-tuple could not enforce. The legacy body answered the same question through four separate task-domain re-entries (`task_asid`, `cr3_for_asid`, `task_status`, `with_tcbs`); the snapshot takes every task-owned field in ONE rank-2 acquisition and resolves the ASID to a page-table root only after that guard is released, because `PAGE_TABLE_STATE` is an independent, unranked lock. Every refusal is unchanged: an absent TCB, a missing ASID and an ASID with no CR3 each yield `None`; absent TLS is `fs_base = 0`; `Blocked`/`Faulted`/`Exited`/`Dead`/`Reserved` and an incomplete saved frame all clear `runnable_saved`. The PREEMPT-PREFER mutation — `with_cpu(cpu, |k| k.on_preempt_prefer_on_cpu(cpu, client_tid))` — moved onto ONE new rank-1 transaction, `on_preempt_prefer_on_cpu_split`, running the SAME `Scheduler::on_preempt_prefer_on` primitive the retired `KernelState` wrapper ran, so the policy still has exactly one implementation. **The legacy fallback is preserved deliberately and was not "improved":** when the preferred TID is not queued on that CPU the scheduler may still select another runnable task and return it, and the caller's `made_current != Some(client_tid)` comparison is what rejects that — unchanged. An invalid or offline CPU still yields `None` with no mutation, reached now by the scheduler's own `check_online_cpu` rather than by `with_cpu` refusing and the caller's `.unwrap_or(None)` collapsing the error. Neither transaction touches the process-global ambient `scheduler.current_cpu`: both are named by the explicit `cpu` the caller trapped on. Identity is NOT strengthened — this path holds only a numeric `client_tid` and the ASID the snapshot discovers, carries no generation-bearing incarnation token, and no `DispatchMarkToken` or generation authority was fabricated for it. The consumer's 8-step ordering is byte-for-byte intact (reply/client gates → read-only snapshot → reject a partial frame → scheduler mutation → require `selected == client_tid` → `DONE` once → clear pending + marker → lock-free MSR/frame/CR3/iret); in particular the context snapshot was NOT moved after the scheduler mutation, and every domain guard is released before `DONE.swap`, the `X86_BSP_SAVED_DISPATCH_OK` marker, `configure_syscall_msrs_for_self`, the FS-base WRMSR, the CR3 write, the frame construction and `resume_user_mode_iret`. Both caller-free `KernelState` helpers (`ap_saved_resume_context`, `on_preempt_prefer_on_cpu`) were deleted. **`src/arch/x86_64/smp.rs` now holds exactly one acquisition** — the unreached ED-2 next-task placement in `ap_sched_next_or_idle`, retained byte-for-byte with its distinct `Err(_) => None` refusal policy — and **there is no production broad `SharedKernel::with` callsite left anywhere in the tree**. This checkpoint is **DELIVERED**. The live proof for the changed path is five consecutive clean runs of `scripts/qemu-x86_64-ap-cross-cpu-reply-smoke.sh` with twelve named markers exactly once per run; `scripts/qemu-x86_64-ap-saved-return-smoke.sh` exercises the untouched CPU-1 AP-return path and its failure signature reproduced interleaved at exact base `c2b9b77` (head 9/10, base 8/9), so it is recorded as an exact-base flake/deferral and is not claimed green. The full remaining inventory — file, line, enclosing symbol, form, class, role, why it remains and the directive expected to retire it — is re-derived from the final source tree in `doc/KERNEL_UNLOCK_AUDIT.md` §1.4a. U3 finally retired the x86_64 **ED-2** next-task placement in `ap_sched_next_or_idle` — `with_cpu(cpu, |k| match k.enqueue_on_cpu(cpu, next_tid) { Ok(()) => k.dispatch_next_on_cpu(cpu), Err(_) => None }).unwrap_or(None)`, the LAST acquisition in this file. It had been retained as unreachable, but that premise was about the SCRIPTS, not the kernel: every scripted profile that set `yarm.ap_user_dispatch=1` also set the direct-IpcCall oracle, which pins `ap_workload_task_count()` to 1. With the oracle selector OFF the same production workload runs `AP_WORKLOAD_TASKS` = 2 and reaches ED-2, proven live before the edit and again after it. The acquisition is retired onto the EXISTING `enqueue_then_dispatch_on_cpu_split` under `EnqueueRefusalPolicy::Decline` — this site's historical `Err(_) => None` policy expressed as a type, with **no new seam added**. `DispatchAnyway` is forbidden here: on a refusal it can select a DIFFERENT task than the one being placed, which the caller's `placed == Some(next_tid)` comparison would then reject, changing a live path. The same pass removed the shared transaction's `scheduler.current_cpu = cpu` write: that field is ONE process-global selector which `current_tid()` resolves through, so writing it from an off-broad seam retargets ambient readers on other CPUs — the failure measured earlier on the SMP saved-resume path. Every step of the transaction names the CPU explicitly (`validate_online_cpu(cpu)`, `enqueue_on_with_priority(cpu, ..)`, `dispatch_next_on(cpu)`), so the binding was never load-bearing for its result, and both callers carry the CPU and selected TID forward as plain values; `validate_online_cpu` is retained and an invalid CPU yields the same refusal. All markers, the seal-probe activation and the ring-3 entry run with every guard released. **`src/arch/x86_64/smp.rs` now holds ZERO broad acquisitions of any form**, and the census falls **8 → 7**, with `with_broad` still **0**. **U9/203C** moved the PRODUCTION post-switch architectural-state restore off the broad lock at `arch/trap_entry.rs`'s second acquisition. The site had been published as "the D3-fenced D6 controlled-proof restore"; both halves of that label were wrong. It was never D6-exclusive — `post_switch_restore_arch_thread_state` ran inside the same `with_cpu` on every ordinary single-CPU x86_64/AArch64 switch — and the `if is_proof_done` block it shared the acquisition with is not obsolete diagnostics: `D6_SWITCH_A_DONE` is emitted only there, for the D6-SWITCH-A *production* unlocked-switch milestone whose knob runs it with the controlled-proof knob OFF, and `d6_ensure_post_cleanup_task_stacks_mapped` is a functional cross-root stack repair without which a post-cleanup trap faults on a supervisor stack write (Stage 165D/165F/165G). The restore is now `post_switch_restore_arch_thread_state_split`: rank 1 (`post_switch_restore_admit_split` — the `validate_online_cpu` admission AND the `current_cpu` bind `with_cpu` performed, with the current TID read under that same guard, never through the deliberately non-binding `current_tid_split_read`) fully released, then ONE rank-2 acquisition (x86_64 reuses the EXISTING `owner_revalidation_snapshot_split`, re-keyed from an `OwnerRevalidationSelection` to the bare `tid` it always read out of it; AArch64 reuses the EXISTING `take_thread_restore_facts`) fully released, then the arch apply with NO lock held — `x86_apply_owner_revalidation_restore` on x86_64, carrying the complete pre-IRET CR3 invariant through `ensure_user_return_cr3_split`, and `apply_restored_thread_state` on AArch64, which now has one writer and three producers. The second premise this increment started from was also false: the production restore DOES perform cross-address-space page-table mutation, in `ensure_user_return_cr3`'s `hw_cr3 != task_cr3` repair branch — but it only MAPS, never unmaps, so it owes no shootdown, it reaches the page tables through a module that takes no `KernelState` lock, and U3 already shipped its production-live off-lock twin. RISC-V is not split: its stash gate opens with `!cfg!(target_arch = "riscv64")`, so the drain is statically unreachable there and it keeps its verbatim in-lock restore inside the retained tail. What still holds the lock is `post_switch_restore_broad_tail`, entered only by the D6 cleanup or that unreachable RISC-V arm; an ordinary production switch now enters no broad acquisition here. **The census does not move — `with_cpu` 6, `with_broad` 0, acquisitions 6 (census 6 / 0 / 6); the 3 raw `state_lock` method definitions in `runtime.rs` are separate bookkeeping and are NOT the third census field — and the reason it does not move is D3, not effort:** the cleanup allocates backing frames (`alloc_user_data_frame`, rank 6) and maps them across address spaces, so a split form would have to add `with_memory_split_mut`, fenced by `AI_AGENT_RULES` §14.4 until the lock-free `await_tlb_shootdown_ack` design and its multi-CPU smoke land — the same conclusion U3 recorded when it excluded this site. **U9-D3 (census 6 → 2)** lifted that fence and retired both cohorts. Vector `0xF1` became the SOLE target-side invalidation and generation-matched ACK producer — the trampoline's sched-idle mailbox service block is deleted, so nothing can race the handler — and it records `origin=kernel|user` from its saved `CS` at a CPL-invariant offset, earning real ACKs from BOTH privilege levels (proven live, three identical runs: `kernel_acks=2, user_acks=1`, `DONE result=ok context=user_origin`). §6 then routed `MemoryObject`/`DmaRegion` through the phased composition, which PERFORMS the active-transfer-mapping unmap with real D3 completion, the supervisor report, the rank-6 `cap_refcount` drop and a reclaim gated on every target having acknowledged — unmap before ACK before reclaim, leaving a frame OUT of the allocator rather than recycling it when an ACK does not arrive — so all three ordinary rollback fallbacks are retired (6 → 3). §7 then split the D6 functional stack repair itself: rank 1 for the current TID, ONE rank-2 acquisition for every live task's kernel stack BY VALUE, then a page-table walk with nothing held that takes rank 6 for each frame ALLOCATION alone and releases it before mapping, plus an exact rollback of any frame whose mapping failed (which the broad body leaked). Exact-base comparison across the whole 60-line D6 cleanup family, in both the `D6_SWITCH_PROOF=1` and `D6_SWITCH_A=1` profiles, differs from base `d3032f8` in exactly one line — a kernel code pointer that moved because the code moved (3 → 2). **The two survivors are the terminal broad dispatchers.** **U9-TERMINAL-FINAL §5 (census 2 → 0)** retired those two as well, and the count is now ZERO. The removal is earned on a MEASURED arrival, not on workload zeros: the base tree's RISC-V smoke logged `TERMINAL_BROAD_DISPATCH_ENTER cpu=0 event=Syscall nr=8` on every run, from PM's Stage 29A boot self-probe. §1 made NR 8's pre-lock route TOTAL (invalid `target_pid`/slots settle `InvalidArguments` pre-lock instead of falling through), §3 admitted NR 8 to the AArch64 ABI import list and the RISC-V `split_eligible` whitelist — the two per-port gates that decide whether the shared dispatcher is consulted at all — and the same boot then measured ZERO arrivals with `PM_NR8_SELF_PROBE_OK` still firing. What replaces the acquisitions is `settle_unowned_trap`: one per-event-class policy with two drivers, never a blanket error. Its `Syscall` arm reproduces the broad arm on BOTH axes — `dispatch_syscall`'s `InvalidNumber` written into the caller's own frame and `Ok(())` returned, so the errno reaches userspace rather than halting the kernel, which is the distinction `KernelState::handle_trap`'s own comment records. Its `PageFault` arm keeps `fault_current_task_for_fault`'s `TaskMissing`. The three asynchronous classes answer `Internal` rather than `Ok(())`, because the broad arm's `Ok(())` was earned by the tick, the routing and the EOI it had just performed, and a settlement that cannot perform them must not claim them. The RISC-V driver composes the existing rank-1 `bind_current_cpu_split` for the CPU admission `with_cpu` performed on entry, and the same pass collapsed that bridge's `inner_result` from `Result<Result<(), _>, _>` to one level — the outer `Ok(…)` had been silently discarding every inner error the two comments on it claimed were propagated. |
...
| `src/arch/x86_64/smp.rs` | 2 | 2466, 2688 |
...
- **Stage 193C (IPCSEND ORDINARY-CAP BOUNDARY SPLIT) — DONE and QEMU-PROVEN (live on the
  default boot).** Extends the IpcSend boundary decomposition to an ORDINARY cap-transfer
  send (exactly one transferred cap, NOT a reply cap, NOT a shared-region) to an
  already-recv-v2-blocked receiver, reusing the SAME 188C ordinary-cap producer + executor
  `ipc_reply` already carries. The broad `SpinLock<KernelState>` is NOT retired.
  * **Wiring.** `handle_ipc_send`'s recv-v2 branch now tries, in order: plain split (193A) →
    ordinary-cap split (193C, `try_ipc_send_boundary_split_ordinary_cap_pub`) → legacy
    `complete_blocked_recv_for_waiter`. The ordinary-cap wrapper delegates to the 188C
    `produce_blocked_waiter_ordinary_cap_delivery`; `Ok(false)` (plain / reply-cap / shared /
    no drainer) consumes nothing and falls through, `Err` (envelope consumed + Phase-A fault)
    → `UserMemoryFault`.
  * **Phase A (broad borrow) — no mint/copy/wake.** The producer validates args, confirms the
    receiver is recv-v2-blocked, confirms exactly one ordinary transferred cap, consumes the
    transfer envelope ONCE, pre-validates the payload+meta buffers, and snapshots object /
    rights / delegation-parent (source_tid, source_cap) + payload/meta BY VALUE. No cap is
    materialized, nothing is copied to user, the receiver is not woken.
  * **Phase B/C (trap-entry drain, global lock dropped).** The executor materializes the
    FRESH receiver-local cap through the 186D2/186D3 seam
    (`materialize_received_message_cap_routed_with_delegation_split`, atomic mint + delegation
    link, NO `ipc_state_lock`), encodes the recv-v2 meta with that fresh CapId, copies
    payload+meta through the 186E seam, then clears return regs + waiter slot and wakes the
    receiver once. A user-copy fault rolls the minted cap ALL the way back
    (`rollback_materialized_recv_cap`: revoke + delegation-link removal + refcount drop) so
    nothing leaks; a materialize fault leaves the envelope consumed with no cap minted
    (net-identical to the legacy meta-fault teardown).
  * **Markers** (executor arm gated on a per-CPU `IPC_SEND_CAP_BOUNDARY_ORIGIN` flag so
    reply-origin cap deliveries stay silent): `IPC_SEND_CAP_BOUNDARY_SPLIT_BEGIN` /
    `SNAPSHOT_OK` (Phase A) → `MATERIALIZE_OK` → `USER_COPY_OK` → `WAKE_OK` →
    `SPLIT_DONE result=ok` → one-shot `GLOBAL_LOCK_RETIRE_CLASS_DONE class=IpcSendOrdinaryCap
    result=ok`; `SPLIT_DEFERRED` / `SPLIT_FAIL reason=materialize|user_copy` on the off/fault
    paths.
  * **Authority + one-shot invariants (proven live).** The receiver-local CapId is minted
    fresh through the seam — never the sender-local handle: the oracle child observes
    `recv_cap=65548` while the sender-local handle it transferred is `65539`
    (`cap_is_fresh=1`). The transfer envelope is consumed exactly once (Phase A); the in-lock
    plan stays `IpcSchedulerPlan::None` so only the drain wakes (wake-once). Byte-identical
    payload delivered (`payload_match=1`).
  * **Live — default boot AND controlled oracle.** Unlike the plain slice, a REAL boot
    cap-transfer to a blocked recv-v2 receiver exists, so `class=IpcSendOrdinaryCap` fires on
    every x86_64 boot (`waiter_tid=10004`, clean `SPLIT_DONE result=ok`) with the full server
    chain intact. The `yarm.ipc_send_cap_oracle=1` sub-knob adds a deterministic controlled
    proof, reusing the 193B fork + atomic receiver-block coordination harness (coord cap in
    init startup slot 13, slot 14 empty — mutually exclusive with plain-oracle slot 14 and
    sender-wake slots 13+14). `YARM_IPC_SEND_CAP_ORACLE=1` oracle smoke observes the full
    marker chain + the child's fresh-cap byte-identical recv; rejects
    `IPC_SEND_CAP_BOUNDARY_SPLIT_FAIL` / `DISPATCH_POST_WORK_FAIL`.
  * **Scope.** core + IPC_FINAL (sender-wake) + crash-restart + IpcSendPlain oracle +
    IpcSendOrdinaryCap oracle + smp2/smp4 (armed and knob-off) pass; riscv64 + aarch64 core
    keep full server-chain parity. IpcCall / IpcReply broad paths, reply-cap / shared-region
    IpcSend, the no-waiter enqueue path, RecvSharedV3, VM / spawn / fork / cap-mint, and
