...
Current census: **32** production callsites — **26** through `with_cpu` and **6** through the
broad `with(|state| …)` form — classified **32 runtime-required**, 0 test-only, 0 obsolete,
...
twelve (44 → 39 → 37 → 36 → 34 → 32). The three raw `self.state.lock()` sites are the bodies of `lock` /
...
**U3 — IN PROGRESS, twelve drains retired (44 → 39 → 37 → 36 → 34 → 32).** U3 promotes existing rank-1
...
*Remaining:* roughly 8 drain re-acquisitions, so the U3 target is **32 → approximately 24**.
...
`ExitCurrentTask` acquisitions — and **two** in `riscv64/trap.rs`: its canonical broad trap phase
and the deferred terminal-idle predicate. Neither file is fully drained.
