...
Current census: **21** production callsites — **20** through `with_cpu` and **1** through the
broad `with(|state| …)` form — classified **21 runtime-required**, 0 test-only, 0 obsolete,
...
twenty-three (44 → 39 → 37 → 36 → 34 → 32 → 28 → 25 → 24 → 23 → 22 → 21). The three raw `self.state.lock()` sites are the bodies of `lock` /
