...
Current census: **42** production callsites — **36** through `with_cpu` and **6** through the
broad `with(|state| …)` form — classified **42 runtime-required**, 0 test-only, 0 obsolete,
...
delivered two (44 → 43 → 42). The three raw `self.state.lock()` sites are the bodies of `lock` /
...
**U3 — IN PROGRESS, two drains retired (44 → 43 → 42).** U3 promotes existing rank-1
...
*Delivered so far*, both in `src/arch/riscv64/trap.rs` and both **read-only**:

1. The post-lock **foundation-oracle** current-TID re-read (44 → 43).
2. The **FutexWait no-incoming idle** branch's `current`-is-idle check (43 → 42), which used
   `with_cpu(cpu, |k| matches!(k.current_tid(), None | Some(0))).unwrap_or(true)`.

Both now read the same per-CPU scheduler slot through the authoritative rank-1 seam
`SharedKernel::current_tid_split_read(cpu)`, with no broad-lock fallback. Source position is
the lock-drop proof in each case: Phase 2's `with_cpu(cpu, …)` closure has already returned.
`src/arch/riscv64/trap.rs` moved 8 → 6 acquisitions.

*Scope of that claim, exactly:* the seam is authoritative for **those two post-lock RISC-V
reads**. It is **not** claimed authoritative for x86_64 pre-lock entering/exiting identity
(which still requires `current_tid_authoritative`), for `current_tid_authoritative` itself, for
dispatch mutation, task status, address-space/frame restore, or any other drain. In particular
**RISC-V `FutexWait` is not off-lock**: its real switch branch still re-acquires `with_cpu` to
write SATP and restore the incoming frame, and that branch was untouched. The analogous AArch64
idle read was also left alone.

*Remaining:* roughly 18 drain re-acquisitions, so the U3 target is **42 → approximately 24**.
...
**Safety fact that U1, U2 and the U3 increments so far preserved, and the rest of U3 must preserve.** `ipccall_direct_production_enabled()`
