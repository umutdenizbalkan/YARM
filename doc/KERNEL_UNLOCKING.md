...
Current census: **37** production callsites — **31** through `with_cpu` and **6** through the
broad `with(|state| …)` form — classified **37 runtime-required**, 0 test-only, 0 obsolete,
...
seven (44 → 39 → 37). The three raw `self.state.lock()` sites are the bodies of `lock` /
...
**U3 — IN PROGRESS, seven drains retired (44 → 39 → 37).** U3 promotes existing rank-1
...
or for any other drain. **Three acquisitions remain in `riscv64/trap.rs`** and 11 in
`trap_entry.rs`.
...
*Remaining:* roughly 13 drain re-acquisitions, so the U3 target is **37 → approximately 24**.
Nine acquisitions remain in `trap_entry.rs` and three in `riscv64/trap.rs`.
