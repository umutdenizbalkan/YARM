...
This file has exactly one status page: §0.3.
...
that site's D3-fenced broad tail remains. Retiring the drain class was necessary for 203C,
not sufficient for it. The post-lock drain class is fully
retired; what remains is the authoritative trap dispatch itself (2), the
production post-switch architectural-state restore (ordinary x86_64/AArch64 path split and lock-free; retained broad tail for functional D6 stack-repair cleanup) (1 — see U9/203C for the label correction) and the recv/delivery boundary (4) — enumerated site by site in
...
9. ~~**AArch64 re-acquires the broad lock on its split return path**
   (`src/arch/trap_entry.rs:1432`).~~ **RESOLVED.** Stage 199D retired it;
   `finalize_split_handled_syscall` takes no broad acquisition today (verified against the
   final tree), and the site is absent from the §1.4a inventory.
...
**The second AArch64 acquisition in that consumer — the replacement task's arch restore — was
deliberately untouched by that pass** and is retired by the next one, below.
`src/arch/trap_entry.rs` falls 5 → 4 `with_cpu` callsites; the census falls **13 → 12**
(**11** `with_cpu` + **1** broad `with`).
...
reacquisition is retired. CANONICAL 203C — still OPEN. CENSUS-DELTA 12 → 11.**

The second and last acquisition of the AArch64 post-lock exit consumer. It held the broad guard
across `d2_recv_switch_incoming_asid(next)` and `post_switch_restore_arch_thread_state`, which
means it performed the TTBR0_EL1/ASID switch and every trap-frame write while holding the whole
kernel. It is now one authoritative exact-identity transaction plus a purely off-lock apply:

* `SharedKernel::post_exit_replacement_restore_split` authenticates the SAME `validate_online_cpu`
  admission predicate `with_cpu` used, binds `current_cpu` the same way, resolves the replacement
  on the EXACT trapping CPU, and takes its ASID, saved user context, pending TLS-restore request
  and parked blocked-syscall completion as ONE coherent rank-1 → rank-2 observation — rank 1
  acquired once, rank 2 nested inside it once, canonical ascending order. The retired body reached
  those facts through four separate re-entrant domain acquisitions underneath the broad guard, so
  the ASID it activated and the context it restored could describe different instants.
* `aarch64::trap::post_exit_restore_replacement` then performs the address-space activation and
  the frame work with EVERY domain lock released, through the SAME single frame writer the in-lock
  restore uses. `restore_arch_thread_state` was split into a gather half and that writer, so the
  completion-before-argument-mirror ordering, the AArch64 error-lane convention and the TLS lane
  have exactly one owner. No restore or completion policy was cloned into `runtime.rs`, and no
  `DispatchMarkToken` was fabricated: the transaction is keyed on `{tid, asid}` alone.

Preserved outcome for outcome: the activation is the same `hal_adapters::switch_address_space` +
`note_address_space_activated` pair `Hal::switch_address_space` performs, so every `active_asid_on`
consumer observes it identically; a replacement with no ASID — including an absent TCB — still
activates nothing, restores nothing, consumes nothing and logs `SCHED_ENTER_IDLE`; a caller with
no trap frame still gets the address-space switch and still consumes nothing; an invalid or offline
CPU yields the identical `KernelError` through the identical `map_err`; and the exiting task is
never a source of restore state. Two refusals are deliberate tightenings, both fail-closed onto the
restore's existing `TaskMissing` class: `next == exiting tid`, and a replacement that is no longer
this CPU's current task.

**The D6 controlled-switch proof acquisition in the same file is EXCLUDED and left byte-for-byte
unchanged.** Its body carries more than a restore: `d6_emit_proof_cleanup_arch_markers`,
`d6_check_asid1_stack_page_mapped` and `d6_ensure_post_cleanup_task_stacks_mapped`, the last of
which maps kernel stack pages into the active root **and every live task root**. That is
cross-address-space page-table mutation, D3-fenced by `AI_AGENT_RULES` §14.4, and no split form
exists for any of the three. Splitting the site into a narrow restore plus a broad cleanup would
leave a broad drain behind at census delta 0, so it was not attempted.

`src/arch/trap_entry.rs` falls 4 → 3 `with_cpu` callsites — re-derived mechanically from source.
The `5` the owner tables carried for that file was **already stale at the base commit**, one
behind the validation retirement above; it is corrected here rather than carried forward, and the
correction is bookkeeping only — it changes no total, because the totals were always taken from
`tests/broad_lock_census_guard.rs`, which recomputes them from source. The census falls
**12 → 11**: `with_cpu` **11 → 10**, broad `with` **stays 1**, total and runtime-required
**12 → 11**, all runtime-required.

**Not live-proven, and not claimed to be.**
`scripts/qemu-aarch64-exit-current-task-smoke.sh` fails at this checkpoint with a mechanically
identical 35-line failure set and the identical
`STAGE_200D0C2_AARCH64_EXIT_CURRENT_TASK_LIVE_SEAL … live_cells=0 result=fail` seal at both the
delivered head and the exact base `3943472`. The oracle workload never activates at either
revision — every `EXIT_TASK_*` marker count is 0 — while the boot itself completes. That is an
**exact-base deferral**, nothing more: it is not evidence for the changed restore path, and no
live cell is earned. The evidence that does transfer is the hosted matrix (4660 single-threaded
lib tests, all 16 integration targets, the census guard and the documentation guards), the three
freestanding kernel builds, the x86_64 / AArch64 / RISC-V standard core smokes — all PASS — and a
clippy comparison against the exact base that is byte-identical.

**203C remains OPEN** — the canonical broad Phase-2 trap handler, the AArch64 FutexWait
no-incoming idle read and the D6 proof restore are still there, and the stage closes only when no
drain-classified acquisition remains. **199C remains OPEN. 199E remains DELIVERED / CLOSED.
200B/U5 remains OPEN (partially production-wired).** Directive U8 is already source-complete —
...
the **canonical broad Phase-2 trap dispatch** and the **production post-switch architectural-state restore, whose ordinary x86_64/AArch64 path is split and lock-free, with a retained broad tail for functional D6 stack-repair cleanup**, the latter
...
* **The two acquisitions in `c2c_bsp_saved_frame_resume` remained present and unchanged** at
  that checkpoint. **The census remained 10** (`with_cpu` 9, broad `with` 1). Both have since
  been retired — see the next entry.

**U3 (canonical 203C) — DELIVERED. The x86_64 BSP SAVED-RESUME COHORT is retired.
CENSUS 10 → 8. `with_broad` 1 → 0. Canonical 203C remains OPEN.**

Both broad acquisitions in `c2c_bsp_saved_frame_resume` are gone, retired in one pass against
the now repeatedly live-reached path.

* **Saved context now uses the authoritative rank-2 snapshot.**
  `shared.with(|k| k.ap_saved_resume_context(client_tid))` — the last production broad
  `SharedKernel::with` in the tree — now calls the EXISTING
  `SharedKernel::ap_saved_resume_context_split`, the same transaction the AP site already ran,
  so no second reader was created. It is destructured by NAME
  (`ApSavedResumeContext { asid, cr3, rip, rsp, gprs, fs_base, runnable_saved }`). The legacy
  body answered the same question through four separate task-domain re-entries (`task_asid`,
  `cr3_for_asid`, `task_status`, `with_tcbs`); the snapshot takes every task-owned field in ONE
  rank-2 acquisition and resolves the ASID to a page-table root only after that guard is
  released, because `PAGE_TABLE_STATE` is an independent, unranked lock.
* **Scheduler preempt/prefer is one explicit-CPU rank-1 transaction.**
  `with_cpu(cpu, |k| k.on_preempt_prefer_on_cpu(cpu, client_tid))` now calls
  `SharedKernel::on_preempt_prefer_on_cpu_split`, which runs the SAME
  `Scheduler::on_preempt_prefer_on` implementation the retired `KernelState` wrapper ran — that
  wrapper immediately re-entered the rank-1 scheduler lock anyway. The policy keeps exactly one
  implementation. Neither transaction reads or writes the process-global ambient
  `scheduler.current_cpu`; both are named by the explicit `cpu` the caller trapped on.
* **Snapshot-before-selection and the legacy fallback semantics are preserved.** The read-only
  snapshot still precedes the scheduler mutation — it was not reordered after it — and when the
  preferred TID is not queued on that CPU the scheduler may still select another runnable task
  and return it. That fallback is preserved verbatim, not "improved": the caller's
  `made_current != Some(client_tid)` comparison is what rejects it. An invalid or offline CPU
  still yields `None` with no mutation.
* **All hardware/frame/iret work is lock-free.** Every domain guard is released before
  `DONE.swap`, the `X86_BSP_SAVED_DISPATCH_OK` marker, `configure_syscall_msrs_for_self`, the
  FS-base WRMSR, the CR3 write, the frame construction and `resume_user_mode_iret`.
* **No stronger incarnation authority is claimed than the caller possesses.** This path holds
  only a numeric `client_tid` and the ASID the snapshot discovers; it carries no
  generation-bearing incarnation token, and no `DispatchMarkToken` or generation authority was
  fabricated for it.
* **Both caller-free `KernelState` wrappers were deleted** — `ap_saved_resume_context` and
  `on_preempt_prefer_on_cpu` — only after confirming neither had any remaining caller.
  `src/arch/x86_64/smp.rs` now holds exactly one acquisition: the unreached ED-2 next-task
  placement in `ap_sched_next_or_idle`, retained byte-for-byte with its `Err(_) => None` policy.
* **Census: `with_broad` 1 → 0, `with_cpu` 9 → 8, runtime-required/total 10 → 8.** The full
  remaining eight-site inventory is re-derived from the final source tree in
  `doc/KERNEL_UNLOCK_AUDIT.md` §1.4a, site by site.
* **Live proof.** Five consecutive clean runs of
  `scripts/qemu-x86_64-ap-cross-cpu-reply-smoke.sh` against freshly built matched artifacts,
  twelve named markers exactly once per run, including
  `X86_BSP_SAVED_DISPATCH_OK cpu=0 mode=saved`. **These five seals are the live proof for this
  changed path.** `scripts/qemu-x86_64-ap-saved-return-smoke.sh` exercises the untouched CPU-1
  AP-return path; its failure signature reproduced interleaved at exact base `c2b9b77` (head
  9/10, base 8/9, same assertion and marker counts), so it is recorded as an **exact-base
...
* **`src/arch/x86_64/smp.rs` now holds zero broad acquisitions**, and **no post-lock drain
  ...
  trap dispatches, one production post-switch architectural-state restore (ordinary x86_64/AArch64 path split and lock-free; retained broad tail for functional D6 stack-repair cleanup), one recv Phase-A multi-domain composite
...
* **203C remains OPEN / PARTIAL** because scheduler operations still execute inside those two
  authoritative trap dispatches and that site's D3-fenced broad tail remains. Direct production remains
  **OFF**.
...
**CENSUS-DELTA 0 — census 6 / 0 / 6** (`with_cpu` 6, `with_broad` 0, acquisitions 6; the 3 raw
`state_lock` method definitions in `runtime.rs` are separate bookkeeping and are NOT the third
census field). `src/arch/trap_entry.rs` keeps exactly two `with_cpu` callsites.
...
profiles. That is a script/kernel contradiction provable from source, not a regression —
**neither D6 profile is green as a whole, and neither is claimed to be**; what is claimed is that
their restore/cleanup gate output is byte-identical to base. The
...
post-switch restore lived inside it — and it wrongly implied the whole acquisition was a
controlled-proof artefact. Its source-truth description is **the production post-switch
architectural-state restore, whose ordinary x86_64/AArch64 path is split and lock-free, with a
retained broad tail for functional D6 stack-repair cleanup** (`post_switch_restore_broad_tail`).
That retained tail is still D3-fenced, and is now the only reason the site is broad.

### Checkpoint record

* **Checkpoint status: DELIVERED.** Directive: **U9 / 203C prerequisite**. **CENSUS-DELTA: 0.**
* **No acquisition was deleted.** The census is `with_cpu` **6**, `with_broad` **0**,
  acquisitions **6**, runtime-required **6** — unchanged. (The 3 raw `state_lock` method
  definitions in `runtime.rs` are separate bookkeeping and are NOT the third census field.) The
  six sites are: ordinary rollback ×3, this post-switch restore site's retained broad tail ×1,
  and the terminal dispatchers ×2.
* **Ordinary production switches no longer enter the broad acquisition** on x86_64 or AArch64.
  The site is bypassed dynamically; the source-counted tail remains one acquisition.
* **x86_64 ordinary restore is live-proven** (see the evidence section above).
* **AArch64 is compiled and hosted-transaction-proven only.** No existing profile live-reaches
  this drain on AArch64, and no live AArch64 evidence is claimed.
* **`D6_SWITCH_A` and the controlled-proof profiles still require the retained functional
  cleanup.** Its broad tail allocates rank-6 frames and maps stacks across address spaces, so
  splitting it remains fenced by `AI_AGENT_RULES` §14.4 / D3.
* **The three ordinary rollback acquisitions remain independently D3-deferred**, and **the two
  terminal dispatchers remain**. Neither cohort was touched.
* **U9 remains OPEN.** **203C remains at its exact source-derived status (OPEN / PARTIAL).**
  **199C and 199E remain DELIVERED / CLOSED.** **Direct production remains OFF.**
* **Stage arithmetic is unchanged** — no canonical exit moved.
* **RPi5, futex, WA3C2 and direct-IpcCall production were untouched.**
...
**199E TIMEOUT-UNLOCKING CHECKPOINT — DELIVERED. CANONICAL 199E — OPEN. CENSUS-DELTA 0.**

> Two things U7 left open are now implemented, and one is now genuinely production-wired.
>
> **Reply/call timeout has production registration and exact retirement.**
> `arm_production_reply_deadline` runs at the committed reply-receive block point on every build
> and boot, driven only by the caller's own `timeout_ticks` — no new syscall, no new ABI field,
> no invented deadline. It registers only when a live reply record is already bound to this exact
> `{caller_tid, caller_asid}` at this exact reply endpoint, so an ordinary receive is untouched,
> and it reuses the existing exact token end to end with the blocked-receive generation serving
> as the deadline-token generation. It fails closed on every degenerate case, including a full
> bounded store — a caller whose registration fails keeps its deadline and is served by the
> receive class, so a registry-full arm never leaves an unbounded blocked wait. Both properties
> are proven through production entries: five sequential real call/reply cycles reuse the
> four-slot registry (cycle five could not register if retirement did not free the slot), and a
> fifth concurrent wait against a saturated registry falls back to the receive class and times
> out exactly once while the four registered waits stay intact.
>
> **Retirement is exactly once, on every terminal outcome.**
> `retire_reply_deadline_for_tid` is wired into the single in-lock wake seam, its split twin and
> `mark_task_dead`, and re-validates the handle against its own terminal identity, so a
> replacement incarnation reusing the numeric TID cannot retire another's registration. Before
> this, `disarm_reply_deadline_on_reply_win` had **zero** production callers: enabling
> registration without it would have leaked one Armed slot per completed call and permanently
> exhausted the four-slot registry after four calls.
>
> **Endpoint AND notification receive timeout are off the broad lock.** Both wait kinds block as
> `Blocked(EndpointReceive)`, so both ride the third off-lock class
> (`drain_recv_timeout_post_work`: rank-2 exact `{tid, asid, blocked_recv_generation}` claim
> publishing the timeout fact in the same acquisition as `Runnable`, rank-3 identity waiter
> removal across every structure plus the stranded re-check, rank-1 single enqueue). The
> broad-lock scan now owns **no** timeout class.
>
> **The deadline clock follows the arming site, not the architecture.** Production deadlines are
> scheduler ticks on all three ports; the hardware counter is read only while the oracle selector
> is armed, and that selector is written only by boot-cmdline parse, so the domain is immutable
> for the lifetime of every record and the two domains never coexist.
>
> **x86_64 production expiry is LIVE-PROVEN.** Provisioning the proof workload was welded to the
> runtime oracle selector, and that selector also makes `maybe_arm_reply_timeout_oracle` pre-arm a
> confined synthetic deadline which wins the one-registration race — so no live boot had ever
> exercised the production arm. Provisioning is now gated by the COMPILE-TIME feature alone (no new
> knob), which lets the SAME existing client run with the selector OFF. On that boot
> (`--profile x86-none`, `mode=0`): `IPC_REPLY_TIMEOUT_ORACLE_PROVISION_OK … mode=0` →
> `IPC_REPLY_TIMEOUT_ARMED … record_generation=17 token_generation=17` (the production arm keys the
> token generation to the blocked-receive generation; the oracle's is the constant `1`) →
> `IPC_REPLY_TIMEOUT_OK … terminal=Timeout timeout_result=TimedOut` →
> `GLOBAL_LOCK_RETIRE_CLASS_DONE class=IpcReplyTimeout` →
> `X86_IPC_REPLY_TIMEOUT_DONE caller_result=TimedOut caller_continuations=1 late_reply=rejected
> result=ok`, alongside `IPC_REPLY_TIMEOUT_LOCK_STATUS … scan_broad_lock=0 … production=1`. Zero
> `ARM_SKIP`/`ARM_FAIL`, zero stranded waiters, zero real fatal/panic/page-fault, and the 15
> graduated-path markers present. The selector-ON retirement cell still passes on the SAME
> artifacts, with `token_generation=1` — the pre-arm proof is preserved and isolated.
>
> **Mixed production/oracle clock domains are repaired.** Making production registration live
> created a hazard the pipeline did not previously have: a selector-on boot now holds BOTH a
> confined oracle record armed from the architectural hardware counter AND ordinary production
> records armed as `scheduler_tick_now() + timeout_ticks`, and the live AArch64/RISC-V logs proved
> it empirically (an unrelated production caller at `deadline=1` beside the oracle's own record).
> The two values are orders of magnitude apart, so a selector-global "reply now" is wrong in both
> directions — a hardware counter makes every production tick deadline instantly due, and a stuck
> tick makes the oracle's counter deadline never due. Each registration therefore carries its own
> `ReplyDeadlineClock` (`ProductionTick` | `OracleHardware`), written by the single registration
> seam `register_reply_receive_deadline` together with the deadline and the token, and never
> rewritten while that registration is live — so selector state cannot reinterpret an already-armed
> production deadline. `run_due_ipc_timeout_work` reads both domains plus the unchanged `send_now`,
> and the collector and the reply drain each select per record from the record's own stored domain.
> No allocation, no ABI change, no broad-lock fallback, no second drain, no per-architecture policy.
> Six focused tests cover coexistence, a huge counter failing to expire a non-due production
> record, a due oracle record expiring while a non-due production record survives, a due production
> record expiring on scheduler ticks alone, repeated scans settling each exact record once, and a
> stale record refusing to be settled across domains.
>
> **Oracle accounting is scoped to the exact oracle identity.** The `IPC_REPLY_TIMEOUT_ARMED` and
> `_OK` marker FAMILIES stopped being oracle-specific the moment production registration went live.
> The retirement profiles now take the oracle's caller identity from
> `IPC_REPLY_TIMEOUT_ORACLE_PROVISION_OK init_tid=` and require exactly one registration for it —
> stricter than the family count it replaces — while bounding settlements by registrations and
> requiring every settlement line to carry its full field contract. Unrelated production settlement
> proceeds normally and consumes no oracle one-shot state; production registration is skipped on no
> endpoint. `IPC_REPLY_TIMEOUT_ARMED arch=` moved from the feature-OFF forbidden set to the
> production-present set on all three ports, because `arm_production_reply_deadline` is
> unconditional production code and its format string is therefore in every image.
>
> **Delivery, stated exactly.** All static gates pass at this commit: integration targets (18
> targets, 0 failures), `cargo metadata --locked`, `cargo check --workspace` (0 errors), the
> canonical hosted suite (4545 pass / 0 fail), `cargo fmt --all --check`, and the census and
> documentation guards. Repository-policy clippy carries the SAME three errors as exact base
> `8275c927` — the pre-existing `yarm-freestanding-alloc` lib-test `uninit_vec` pair, deferred as
> exact-base — and +7 warnings across six families the base already carries at 14–97 instances
> each. One candidate finding was a real signal and was FIXED (`recv_timeout_work_clear` was dead
> under a plain hosted build; it is now gated exactly like its `reply_timeout_work_clear` sibling).
> The remaining six are ACCEPTED under repository policy, the production one with an in-source
> rationale: the retired-but-preserved `process_ipc_timeout_deadlines` body still names `deadline`
> at a use the unconditional skips make unreachable, so clippy reports the binding unused while
> rustc still resolves it — deleting the binding would mean editing the preserved body, and an `_`
> prefix does not compile against that same use.
>
> Fresh matched standard artifacts for all three architectures attest this commit with
> `ARTIFACT_BUILD_INTEGRITY … stale_artifact_acceptance=0 failed_build_rejected=1 result=ok`, and
> the established standard core profile passes on x86_64, AArch64 and RISC-V with zero fatal, zero
> panic, zero duplicate wake and zero stale completion. AArch64 core carries the pre-existing
> `PAGE_FAULT_UNHANDLED tid=1 addr=0x0 access=Read rip=0x40307c` reproduced character-identically at
> exact base, and its own gate passes.
>
> **x86_64 is fully green.** The retirement profile passes both cells, and selector-off production
> expiry is live-proven on fresh feature-on artifacts. The clock domain is proven by the ARM
> SIGNATURE rather than asserted: for the same caller and the same record generation 17, the
> selector-off boot registers `token_generation=17` — the production arm keys the token generation
> to the blocked-receive generation, so this is `ProductionTick` — while the selector-on boot
> registers `token_generation=1`, the oracle's constant, so that is `OracleHardware`. The
> selector-off cell settles `ARMED=2 OK=2 COMMITTED=2`: one settlement per registration, exactly one
> completion, wake and enqueue per identity, with `ARM_SKIP=0`, `ARM_FAIL=0`, zero stranded waiters,
> zero leaked deadline/record/waiter/lease, zero fatal/panic/page-fault, and all 15 graduated-path
> markers present. Oracle accounting is scoped to the exact oracle identity throughout.
>
> **AArch64 and RISC-V are DEFERRED, and only because each is mechanically exact-base-identical.**
> The AArch64 oracle client workload never starts, here and at base alike — `ordered marker sequence
> incomplete (ci= cn= rt= ud=)` character-identical, client-start markers 0. On RISC-V the clock
> repair restored the client's full sequence (`ci=29552 cn=29654 rt=29657`, where before the fix it
> stalled at `ci` with a duplicate timeout encoding) and the candidate's own settlement is
> exact-once — `ARMED=2 OK=1 COMMITTED=1 CLASS_DONE=1`, the oracle record settling while the
> unrelated production record correctly does not — leaving only the client's
> `caller_result=1 caller_continuations=2 late_reply=1 result=fail`, character-identical to base.
...
>
> **What stays open.** RISC-V's S-mode timer bridge prerequisite is now live-proven under the
> opt-in `riscv64-timer-irq` feature — see the 199E-R1 entry below and `doc/ARCH_RISCV64.md` §13.6 —
> but the feature stays default-OFF and **canonical 199E remains OPEN pending RISC-V.** Census delta 0; direct
...
> **199E-R1 — the RISC-V S-mode supervisor-timer bridge, proven under the opt-in feature.**
>
> RISC-V was 199E's sole remaining default-reachability blocker: a timer interrupt arriving from the
> kernel's idle `wfi` re-entered the bridge as S-mode and hit
> `RISCV_TRAP_UNHANDLED reason=trap_from_s_mode`, so `sstatus.SIE` was never armed and the shared
> scheduler counter never advanced.
>
> The audit invariant is one sentence: **`sstatus.SIE` is set at exactly ONE program point, and the
> only S-mode code that can be interrupted with it set is `riscv_trap_halt`'s `wfi` loop.** Hardware
> clears SIE on every trap entry, so no syscall or fault handler is interruptible; U-mode routing is
> gated by privilege rather than SIE, so it is untouched; and SIE is enabled last, immediately before
> the kernel commits to the never-returning idle loop, with no lock held —
> `handle_riscv_trap_entry_shared` has already returned by value, its bounded `with_cpu` guard
> dropped and its post-lock work drained. The bridge CHECKS this rather than assuming it: interrupt
> bit, supervisor-timer cause, `SPP`=Supervisor, and a boundary latch armed at that one point.
> Everything else stays fail-closed on the pre-existing halt, and a genuine error inside the accepted
> path halts rather than resuming as idle.
>
> Per accepted interrupt: **one** tick through the arch-neutral `Trap::TimerInterrupt` pipeline —
> reusing the SAME shared wrapper the user path uses, so no broad-lock callsite is added and no
> second timeout drain exists — then **one** SBI `set_timer`, which on RISC-V is both acknowledgement
> and re-arm (there is no separate end-of-interrupt). Boot hart only. `sscratch` is re-pointed at the
> true trap-stack top at the boundary and again on every S-mode return, so each accepted interrupt
> lands its frame at a fixed address instead of inheriting the generic tail's drifting value.
>
> Two corrections were required. `acknowledge_interrupt(0)` reached the PLIC's claim/complete
> register and faulted (`scause=0xf stval=0x0c201004`) — PLIC source 0 is reserved and a supervisor
> timer is not a PLIC source, so it is now refused. And the blocked-syscall no-provenance defensive
> guard is scoped to syscalls: a timer interrupt at terminal idle legitimately observes no provenance
> and no current task.
>
> **Live, feature-on:** 10 accepted interrupts, 10 re-arms, strictly monotonic
> `YARM_SCHED_TICK cpu=0`, boot hart the sole owner, zero unhandled S-mode traps, no storm, and a
> task woken from terminal idle dispatched back into U-mode. **Default builds unchanged**:
> `feature=0`, `reason=timer_irq_feature_disabled`, tick 0.
>
> **The two defects the live bridge exposed are repaired.**
>
> *(A) D2 blocked-receive return provenance.* The U4 D2 in-lock bypass returns before the
> arch-neutral syscall-return classification that publishes idle provenance, so a deferred receive
> never published a token and the wrapper's terminal-idle handling took the defensive error path.
> Publication now happens at the **authoritative D2 completion point**: the post-lock drain's clean
> idle outcome — the point the bypass diverted control away from, and the one its own comment says
> should produce the typed idle outcome. Everything the token asserts is established there: exact
> identity and generation (`d2_recv_reverify_blocked` gates the branch, so a replacement incarnation
> reusing the tid cannot publish), a committed block, a clean `Mark::Idle` (both refusal arms and the
> torn arm publish nothing), an actual pending deferral (nonblocking and immediate receives reach
> nothing), exactly once, and outside incompatible locks. `RISCV_BLOCKED_IDLE_NO_PROVENANCE` is NOT
> weakened, nothing is synthesized in the timer handler, and nothing is inferred from a bare tid or
> `current`. The repair is RISC-V-local because RISC-V is the sole reader of the token.
>
...
> `sscratch` and then obtained sp via `csrrw sp, sscratch, sp`, at which point sp was the BRIDGE's
> pointer (the tail is reached by a `call`). The swap left `sscratch` one frame below the frame base,
> so the next trap started `304 + |bridge frame|` bytes lower — every trap, monotonically. The return
> now writes the canonical per-hart trap-stack top and loads the interrupted sp directly from the
> frame's saved `x2`: the interrupted sp round-trips exactly, the next entry always starts from a
> fixed address, the top is resolved per-hart from the frame pointer so ownership cannot alias, and
> the task-switch ABI is unchanged. Live-verified over 800+ traps with a temporary diagnostic —
> identical frame address on every sample — which was then removed.
>
...
> `scan_broad_lock=0 production=1`, and a task woken from terminal idle returned to U-mode.
>
> **Still open.** The production reply deadline is armed in the `ProductionTick` domain and the
> collector/drain runs every tick, but the supervisor's reply legitimately wins and the record is
> correctly disarmed, so `TimedOut` does not fire — the same intended outcome the AArch64 checkpoint
> records.
>
> **The opt-in timer is now periodic rather than idle-only.** `program_timer_deadline` is the ONE
> common re-arm point, at the tail of the arch-neutral `Trap::TimerInterrupt` arm that BOTH accepted
> origins pass through, so U-origin and audited S-idle-origin interrupts re-arm through the same
> seam exactly once each; the duplicate S-mode-local re-arm was removed. SBI `set_timer` both clears
> the pending condition and programs the next deadline, so it is the whole completion. Boot hart
> only, no PLIC, and a complete no-op with the feature off. Fixed alongside it: `sstatus.SIE` was set
> only on the FIRST idle entry, so every later arrival — coming from a trap handler where hardware
> cleared it — left the idle `wfi` masked and ticks stopped as soon as anything was dispatched;
> `reestablish_idle_boundary` re-enables it at the SAME audited boundary, gated on `stie_enabled()`.
> Live: 480 interrupts = 480 re-arms, strict 1:1, no storm or drift.
>
> **Both gaps are closed by a profile-confined selector-off proof cell, and a third defect it
> exposed is repaired.**
>
> *The blocking proof handshake.* The oracle lane arms an `OracleHardware` deadline up front against
> a clock that advances on traps, so its server may yield-spin until the client times out — the
> spin's own traps advance the very clock it waits on. The production lane cannot: its deadline is
> armed by the ordinary block path and expires against the scheduler tick, which here is driven by
> the periodic timer armed at the terminal kernel-idle boundary, so a spinning server keeps a
> runnable task on the CPU, idle is never reached and the deadline can never come due (measured
> deterministically). The production lane therefore BLOCKS both parties — client on a real finite
> reply deadline, server on a second receive on the EXISTING request endpoint while retaining the
> reply capability — which is exactly the state the production timeout is specified to resolve.
> Retaining the capability across that receive adds no resource: `ipc_send` passes no reply endpoint,
> so the trigger mints no second reply capability and cannot alias the retained one, and the server
> asserts both halves on receipt. The two lanes are told apart by `ProductionTimeoutWins`, a fourth
> value appended to each architecture's existing slot-5 liveness run (base+3) with symmetric
> encode/decode — proof-protocol metadata in the same slot, no ABI slot and no capability added.
> Only RISC-V publishes it; x86_64 keeps the shared helper and AArch64 the bare selector.
>
> *Live, selector-off, feature-on:* `SERVER_REQUEST_CONSUMED` → `SERVER_BLOCKED_FOR_LATE_REPLY_TRIGGER`
> → `KERNEL_IDLE` → `STIE_ENABLED` → S-origin ticks → `IPC_REPLY_TIMEOUT_ARMED … deadline=3` →
> `DEFERRED published=1 drained=1` → `OK terminal=Timeout timeout_result=TimedOut caller_wakes=1` →
> `COMPLETION_COMMITTED` → `GLOBAL_LOCK_RETIRE_CLASS_DONE class=IpcReplyTimeout` → `CLIENT_TIMED_OUT`
> → `CLIENT_DWELL_BEGIN` → `RISCV_U_MODE_TIMER_ACCEPTED … origin=user` → `CLIENT_DWELL_DONE` →
> `LATE_REPLY_TRIGGER` → `SERVER_LATE_REPLY_REJECTED rejected=1`. This is the first live witness of
> BOTH a U-mode-origin timer interrupt and a real non-oracle `ProductionTick` reply-deadline expiry.
>
> **199E-R1D — asynchronous user-context save/restore.** The dwell exposed a latent whole-port
> defect nothing had been able to reach before: no path captured a user register file interrupted
> ASYNCHRONOUSLY. `Trap::Syscall` snapshots via `sync_current_thread_from_frame`; the timer arm never
> did, so a preempted task's TCB still held its last `ecall`'s state and a switch away and back
> resumed it there — measured as a caller re-running its receive continuation 75 times. Both existing
> resume conventions are lane conventions keyed to a syscall boundary (startup installs `a0..a5` from
> the argument mirror and zeroes `a7`; a continuation installs its result lane), and neither is
> correct where `a0..a7` are ordinary live registers.
>
> RISC-V now has an explicit THIRD resume state. The snapshot is taken on a U-origin timer trap
> strictly before anything that can schedule, tagged with the exact `{tid, asid, preempt_generation}`
> incarnation — `checked_add`, context written before the tag, so a tag can never name a half-written
> register file — and consumed exactly once at whichever resume boundary runs, in-lock or post-lock.
> The consuming write-back restores the complete integer register file including `a0..a7` and never
> passes through the startup rewrite; the decision travels to it through the same per-CPU flag family
> the other five cross-phase decisions use, so no broad-lock callsite is added. A replacement
> incarnation reusing the numeric TID, a superseded generation, or a missing tag all fail closed —
> never a fallback to startup arguments — and exit clears the tag. Nothing is inferred from zero
> registers, a PC value, a bare TID or incidental scheduler state. Fresh, syscall-return and D2 paths
> are byte-for-byte unchanged, and the canonical `sscratch` stack-top invariant is retained.
>
> *Live:* five real switch-away/switch-back cycles inside one dwell, each restoring the same
> `sepc`, user `sp`, `a0=0xa0a00001dead0000` and `a7=0xa7a70008dead0007` with `startup_rewrite=0`; an
> assembly register canary pinning `a0..a7` as operands reports `mismatches=0x0000 result=ok`;
...
*Delivered so far*, all in `src/arch/riscv64/trap.rs`, taking that file 8 → 3 acquisitions:

1. The post-lock **foundation-oracle** current-TID re-read (44 → 43).
2. The **FutexWait no-incoming idle** branch's `current`-is-idle check (43 → 42). Both now read
   the per-CPU scheduler slot through the rank-1 seam `current_tid_split_read(cpu)`.
3. The three homologous **switch/restore** drains — **queue-switch foundation**, **FutexWait
   switch-success** and **Yield switch-success** (42 → 39). Each retained its exact
   `DispatchMarkToken` and now runs one shared exact-token transaction:
   `direct_dispatch_activate_asid_split` → `direct_dispatch_restore_context_split` →
   (feature-gated) `direct_dispatch_take_completion_split`.

The real RISC-V hardware sequence was **preserved, not deferred**: `switch_address_space`
deliberately defers RISC-V paging, so the activation seam performs the same
`map_kernel_shared_into_asid` → page-table root → `write_satp` (which issues the `sfence.vma`)
on freestanding RISC-V, and every other architecture keeps the generic path unchanged. An
exact-identity refusal rolls the dequeue back through the narrowed authority and diverges — it
never returns through another task's frame, and never fabricates rollback authority for a
`ContinuedCurrent` mark.

*Scope of that claim, exactly:* these five RISC-V drains. Nothing is claimed for x86_64 pre-lock
entering/exiting identity (still `current_tid_authoritative`), for dispatch mutation elsewhere,
or for any other drain. At that point **three acquisitions remained in `riscv64/trap.rs`** and 11 in
`trap_entry.rs`.

Two more were then retired on **AArch64** in `src/arch/trap_entry.rs` (39 → 37, taking that
file 11 → 9): the **FutexWait** and **Yield** switch-success restores now run a neutral
exact-token resume core lifted out of `direct_dispatch_resume_incoming` (which stays a thin
wrapper keeping its own `AARCH64_DIRECT_DISPATCH_*` marker contract for its direct caller).

The **FutexWait no-incoming idle** current-TID read was also retired onto
`current_tid_split_read(cpu)` and then **restored**: its only live acceptance gate is the Stage
195F idle oracle, whose precondition ("all servers up and blocked on recv") is unreachable
...
domains, so they are now taken as ONE coherent transaction —
`post_lock_exit_validation_split` — holding the rank-1 scheduler lock and nesting the rank-2
task acquisition inside it, in canonical ascending rank order. Full `{tid, asid}` incarnation
validation, absent-TCB semantics (identity-safe AND terminal), all-runqueue absence and the
invalid/offline-CPU error are preserved exactly; the snapshot is read-only.

That increment was reduced from a proposed two-drain cohort. The second candidate — the
**blocked-syscall terminal-idle predicate** — was withdrawn *before* any production edit
because baseline reconnaissance at the accepted base found its decision arm has no live
coverage: `RISCV_BLOCKED_IPC_IDLE_PROVENANCE_OK` is absent from the normal core smoke, from
all four SMP configurations of the smoke matrix, and from the exit-oracle boot, every RISC-V
boot reaching terminal idle instead through `FutexWaitNoIncoming`. Nothing is failing there —
`RISCV_BLOCKED_IDLE_NO_PROVENANCE` and `RISCV_TRAP_HANDLE_FAILED` are both zero — the arm
simply never fires, so the substitution could not be live-proven. It is deferred rather than
shipped unverified.

Two more were then retired on **x86_64**, in `src/arch/trap_entry.rs` (36 → 34, taking that
file 9 → 7): the **D2 blocking-send** and **D2 blocking-receive** switch-success restores. Both
were byte-identical bodies that re-acquired the broad lock only to run
`d2_recv_switch_incoming_asid(inc)` plus the arch restore, having already reduced a complete
`DispatchMarkToken` to a bare TID. They now share ONE neutral transaction,
`x86_post_lock_resume_marked_incoming`, which keeps the whole token: it re-validates the rank-1
scheduler still names the token's incoming TID on the token's own authenticated CPU, activates
the token's exact ASID through `direct_dispatch_activate_asid_split`, restores the exact saved
context and takes the pending TLS request through `direct_dispatch_restore_context_split`, then
applies the frame, the FS base and `LAST_RESTORED_TLS_BASE` unchanged.

The x86 pre-IRET CR3 invariant is **preserved whole, not assumed away** by the ASID activation:
`ensure_user_return_cr3_split` still reads the ACTUAL hardware CR3, looks up the target root,
and on a mismatch samples live RIP/RSP, scans every TCB's kernel-stack bounds for the stack
containing RSP — now through the rank-2 seam — calls
`ensure_kernel_return_context_mapped_for_asid`, and writes CR3 only once that mapping is proven,
taking `USER_CR3_PRE_IRET_SKIP` otherwise. A page-table failure is never weakened into success.
The scan predicate and the fallback bounds were lifted into single shared definitions, so the
broad and split lookups are identical by construction rather than by assertion.

A refusal cannot resume a replacement incarnation: the deferral is cleared exactly once, the
dequeue is undone only through `token.into_dequeued_authority()` (a `ContinuedCurrent` mark
yields none, and none is fabricated), and the path then diverges into the established
wake-capable idle halt rather than returning through the outgoing task's frame. No
switch-success marker is emitted after a refusal. `frame == None` keeps its old contract
exactly — activation happens, nothing else does, and it is not an error.

Two more x86_64 restores followed (34 → 32, taking `trap_entry.rs` 7 → 5): the **FutexWait**
and **Yield** switch-success bodies. They were the same body a third and fourth time, so they
**reuse `x86_post_lock_resume_marked_incoming` unchanged** — the transaction was not widened,
extended or re-specified for them, which is the point: four callers, one implementation. Each
retains its own class markers and counters, clears its own deferral exactly once, and on refusal
rolls the dequeue back only through `token.into_dequeued_authority()` before diverging, emitting
no `FRAME_OK`, no counter bump and no success marker. The non-switch outcomes are untouched:
FutexWait's state-changed and idle branches, and Yield's defensive idle branch, restore no frame
and run no extra dispatch.

The four broad `with(|state| …)` acquisitions in `src/runtime.rs` then followed (32 → 28,
taking that file 4 → 0). They are a different shape from the post-lock drains above: the two
home-CPU wrappers moved onto the rank-2 task seam, reproducing `KernelState::task_home_cpu`
and `set_task_home_cpu` exactly (idle TID, absent TCB and unpinned TCB all still answer
`None`; assignment still reports `false` for a missing TCB, adds no online-CPU validation and
changes no placement policy). The reply-win deadline disarm became two SEQUENTIAL narrow
acquisitions — rank 2 for the exact `{caller_tid, caller_asid}` handle read, released in full,
then rank 3 for the exact token disarm — never nested, with neither lock held across a user
copy. The old body already dropped the broad lock between its two `self.with(...)` calls, so
the inter-operation window is unchanged and the token's generation and epoch remain the
stale-disarm protection. No new seam, wrapper, marker or script was added.

*Honest scope of that increment:* the four acquisitions were compiled in production source,
but at the retirement base **none of the three enclosing `SharedKernel` wrappers had a
production caller** — `smp_request_wake_target_split_read` and
`disarm_reply_deadline_on_reply_win` were reached only by hosted tests, and
`smp_assign_task_home_cpu` had no caller at all. Four broad acquisitions left the production
census; four live production paths did **not** move. The underlying `KernelState` methods are
unchanged and keep their other uses.

Three **authoritative current-identity** acquisitions followed (28 → 25): the
`SharedKernel::current_tid_authoritative` body itself (`runtime.rs` 12 → 11) and the two
x86_64 `entering_tid` / `exiting_tid` snapshots in
`src/arch/x86_64/descriptor_tables.rs` (2 → 0, that file now has none).

This is **not** a repeat of the reverted Stage 4T+6 substitution, and the difference is the
whole point. That experiment pointed the snapshots at `current_tid_split_read(cpu)`, which
reads a per-CPU slot but does **not** bind `scheduler_state.current_cpu`; the live x86 service
chain then disappeared (`service_entries=0`, repeated `SCHED_ENTER_IDLE_HLT`). Here the
binding side effect is preserved exactly — `current_tid_authoritative` still validates the CPU
with the same online predicate `set_current_cpu` uses, returns `None` without touching
`current_cpu` on failure, binds `current_cpu = cpu` (even when that CPU has no current task),
and only then reads. All of it now happens inside ONE rank-1 `with_scheduler_split_mut`
acquisition instead of the broad lock. The lookup CPU is resolved as `KernelState::current_tid`
resolved it, which is **not** uniformly the caller's `cpu`: on freestanding AArch64
`current_cpu()` is `MPIDR_EL1`-derived, and that hardware-derived lookup is preserved rather
than silently replaced. `current_tid_split_read` remains intentionally non-binding and is
**not** used by either snapshot. No new lock seam and no new script were added; the two
snapshots keep their exact program positions (entering before dispatch, exiting after dispatch
and AP-seal handling) and `task_switched = entering_tid != exiting_tid` is unchanged.

*Remaining:* roughly 8 drain re-acquisitions, so the U3 target is **25 → approximately 22**.
**Five** acquisitions remain in `trap_entry.rs` — the canonical broad trap phase, the AArch64
FutexWait no-incoming idle read, the post-switch architectural-state restore and the two AArch64
`ExitCurrentTask` acquisitions — **two** in `riscv64/trap.rs` (its canonical broad trap phase
and the deferred terminal-idle predicate), and the **two** broad `with` reads in
`src/arch/x86_64/smp.rs`. Those last two are explicitly deferred: their
`ap_saved_resume_context` transaction combines exact task context with ASID→CR3 resolution and
needs a separate concurrency design. No file above is fully drained except `runtime.rs`, whose
broad `with` set is now empty (its 12 `with_cpu` acquisitions remain).
This is the work canonical **203C** defines; U3 advances it and does **not** complete it —
**203C remains OPEN**, and completing it additionally requires `d6_genuine_enabled()` to stop
being compile-time x86_64-only (§0.6 blocker 3).
...
that fires on every trap must cost O(1) when its precondition is absent. The defect was
INTRODUCED during this increment, BISECTED to it, FIXED, and GUARDED.

### Delivery status, and what is NOT claimed

**The U9-D3 cohort is DELIVERED.** Direct-IpcCall production remains OFF
(`ipccall_direct_production_enabled()` is `const false`).

***U9 itself remains OPEN.*** This cohort closes the D3-fenced work; it does not close U9. Two
production broad acquisitions remain — the terminal dispatchers — and **U9 is not complete until
those two are themselves retired.**

**Six live profiles are red, and all six are exact-base deferrals rather than candidate
regressions**: `qemu-x86_64-core-smoke` under `D6_SWITCH_PROOF=1` and under `D6_SWITCH_A=1` (an
unconditional REMOVE-FALLBACKS gate that mode isolation itself disables by forcing
`UNLOCK_GRADUATED=0`), `qemu-x86_64-server-dies-smoke`, `qemu-aarch64-exit-current-task-smoke`,
`qemu-aarch64-server-dies-smoke`, `qemu-shared-region-direct-aarch64-smoke` and
`qemu-riscv64-server-dies-smoke`. Each was reproduced at base `d3032f8` with a **byte-identical
failure signature** after address normalization.

**Neither D6 profile is claimed globally green.** Their end-to-end verdicts are `result=fail`, for
the pre-existing reason above. What is claimed is only the marker-family equivalence described in
§7 — one differing line out of sixty, `[ok]` counts equal, `[error]` sets identical — which is
exactly what the §7 argument requires and no more.
...
admits it. No unlocking increment may flip that default, as a side effect or otherwise.
...
> **HISTORICAL — NOT A STATUS PAGE.** Mechanisms described here are accurate. For status, see §0.3. For the next action, see §0.7.
...
> **HISTORICAL POINTER — NOT CURRENT.** The Cycle 12 (2026-06-16) "Remaining work" roadmap
> and its Immediate / Next / Concurrent task lists stood here and have been removed: they were
> obsolete, and a stale task list reads as a live plan. Everything numbered §7.x below —
> including the §7.1 readiness audit and its per-stage records — is retained as historical
> evidence only; it describes what was done, not what is current or what to do next.
> **Current status: §0.3. Next action: §0.7.**
...
**Stage 186D2 (CAP-TRANSFER-MATERIALIZATION-SEAM-FIRST-SLICE) — DONE (infrastructure only,
...
(`materialize_received_message_cap_routed` → `materialize_split_transfer_cap_equivalent`)
...
(via the atomic mint) — never `ipc_state_lock`, never a broad `&mut KernelState`.

Added, in `boot/cap_transfer_materialize_split.rs` on `SharedKernel`:

- `TransferCapSnapshot { receiver_cnode, object, rights }` — a plain, IPC-lock-free snapshot
...
- `materialize_received_cap_snapshot_split(snapshot) -> Result<CapId, _>` — mints the ordinary
...
- `materialize_received_message_cap_routed_split(snapshot) -> Result<CapTransferMaterializeOutcome, _>`
  — routes ordinary object caps to the atomic mint (`Materialized(CapId)`) and **reply-cap
...
or reuses an envelope — one-shot semantics preserved at the interface boundary.

**Atomic-mint usage proof.** The seam goes *through* `mint_capability_with_memory_ref_split`
and never bypasses it (no direct cspace `.mint()`, no direct capability/memory seam) — so a
cap is never published without memory-object refcount protection, and the 186D-proper rollback
is preserved. Guarded functionally (memory transfer bumps `cap_refcount` +1; full cnode →
`CapabilityFull` with the bump rolled back).

**Lock-scope / rank proof.** No `ipc_state_lock`, no broad `&mut KernelState`, no cap→IPC
rank inversion — the seam holds only the atomic mint's disjoint rank-6 then rank-4 sections.
...
receiver-local CapId + atomic refcount) but does **not** yet record the source→dest
...
preserved, stale rejected before publish, full cnode → CapabilityFull + rollback, reply
...
docs don't overclaim). Dead/uncalled in release → zero behavior change.
...
(`set_reply_cap_waiter_cap` after mint) — both still deferred. Full global-lock retirement
...
context-switches the BLOCKING caller** — FutexWait (NR 9) — behind a **default-off,
...
- **Eligibility (MANDATORY incoming exists):** armed + shared trap drain + single
  ...
  No incoming ⇒ `RISCV_FUTEX_WAIT_RETIRE_DEFERRED reason=no_incoming`, no consume, no
...
- **In-lock publish:** claim one-shot, `futex_wait_dispatch_try_defer`, skip in-lock
  ...
  BLOCK_PUBLISH_OK}`; publish failure → clear + `..._FALLBACK reason=defer_failed` →
...
- **Post-lock drain:** `futex_wait_reverify_blocked` (rank-2 re-acquire proves the
  broad guard dropped → `LOCK_DROPPED_OK`, AND confirms still `Blocked(Futex)`) →
  dequeue B (rank-1) → set current → mark Running (rank-2) → fresh `with_cpu`: real
  `write_satp` (`csrw satp` + `sfence.vma`) + `restore_arch_thread_state` → `sret`.
...
- **Live oracle** (slot-5=3): A spawns B, blocks on FutexWait → switch to B; B emits
  `RISCV_FUTEX_WAIT_INCOMING_USER_OK`, wakes A via split FutexWake (count 1), parks
  legacy; A resumes once → `RISCV_FUTEX_WAIT_USER_RETURN_OK tid=<A> wake_count=1` +
...
- **Excluded / not yet claimed:** default-on FutexWait + post-lock idle (196F); NR 9
  ...
  green; counts unchanged (32/23); no new kernel lock; 2 MiB trap-stack preserved.
  Next: **196F** — default-on FutexWait + post-lock idle/no-incoming.
