...
The early control-channel bounded poll now emits `SUPERVISOR_CONTROL_RECV_BEGIN cap=... mode=bounded_poll` before the receive. In the gated crash-restart path only, a `WrongObject` from this obsolete optional startup control probe is logged as `SUPERVISOR_CONTROL_RECV_SKIP reason=restart_test_optional_control_probe_wrong_object` instead of the generic `supervisor.srv control recv error: WrongObject` noise. Managed-record mode still skips control waits in favor of fault priority, and the accepted crash-restart path (record-token hit, attempts 1/2/3, old-target reap, and degraded final state) is unchanged. CAPABILITY_MODEL invariants remain unchanged: CapIDs are cspace-local, reply caps are one-shot, and real `WrongObject`/`StaleCapability`/`MissingRight` failures outside the documented optional-probe skips are not converted to success.
