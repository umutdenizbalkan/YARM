...
### SUP-L7L startup optional-probe cleanup

SUP-L7L removes two startup-only WrongObject noise markers from the gated crash-restart path without changing restart policy or PM authority. The supervisor now emits `SUPERVISOR_LIFECYCLE_QUERY_BEGIN tid=... cap_kind=pm_request_reply source=startup` before the optional self lifecycle probe. When the compile-time crash-restart gate is enabled, that self-probe is skipped with `SUPERVISOR_LIFECYCLE_QUERY_SKIP tid=... reason=restart_test_managed_record_probe_obsolete` because managed restart state is established by explicit supervisor registrations, managed records, record-first restart tokens, and PM restart replies. This skip is limited to the optional lifecycle self-probe; PM restart, token query fallback, IPC, and capability errors still remain hard failures.

SUP-L7M corrects the control side of SUP-L7L: the optional startup control probe is skipped before it can call recv (`SUPERVISOR_CONTROL_OPTIONAL_PROBE_SKIP reason=registration_receive_required`), but the required pre-managed-record registration receive still runs and is marked by `SUPERVISOR_CONTROL_REQUIRED_RECV_BEGIN mode=registration` and `SUPERVISOR_CONTROL_REQUIRED_RECV_OK ...` on delivery. A required receive error is logged as `SUPERVISOR_CONTROL_REQUIRED_RECV_ERR err=...` and yields once to avoid a tight startup spin; it is not treated as a registration success and does not bypass `handle_control_request`. Managed-record mode still skips control waits in favor of fault priority, and the accepted crash-restart path (record-token hit, attempts 1/2/3, old-target reap, and degraded final state) is unchanged. CAPABILITY_MODEL invariants remain unchanged: CapIDs are cspace-local, reply caps are one-shot, and real `WrongObject`/`StaleCapability`/`MissingRight` failures outside the documented optional-probe skip are not converted to success.
