...
### SUP-L7L optional startup WrongObject cleanup

SUP-L7L does not change the PM restart contract, PM trusted-supervisor validation, supervisor reply validation, record-token precedence, resource-unavailable retry behavior, or old-target reap placement. The lifecycle self-query remains skipped with `SUPERVISOR_LIFECYCLE_QUERY_SKIP ... reason=restart_test_managed_record_probe_obsolete`. SUP-L7M fixes the control half: the obsolete optional control probe is skipped before recv (`SUPERVISOR_CONTROL_OPTIONAL_PROBE_SKIP reason=registration_receive_required`), while the required registration receive still runs through `SUPERVISOR_CONTROL_REQUIRED_RECV_BEGIN mode=registration` and must reach `handle_control_request` on delivery. PM restart, token query fallback, IPC receive, reply-cap, and capability errors continue to fail closed and must not be hidden as success.
