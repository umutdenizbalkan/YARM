...
SUP-L7L does not change the PM restart contract, PM trusted-supervisor validation, supervisor reply validation, record-token precedence, resource-unavailable retry behavior, or old-target reap placement. It only gates two obsolete supervisor startup probes in the crash-restart build: the supervisor self lifecycle query is skipped with `SUPERVISOR_LIFECYCLE_QUERY_SKIP ... reason=restart_test_managed_record_probe_obsolete`, and the early optional control receive probe maps a crash-test-only `WrongObject` to `SUPERVISOR_CONTROL_RECV_SKIP reason=restart_test_optional_control_probe_wrong_object`. Those skips are documented optional-probe paths; PM restart, token query fallback, IPC receive, reply-cap, and capability errors continue to fail closed and must not be hidden as success.
