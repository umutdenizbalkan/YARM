...
| D6 switch proof | 🧪 default-off `yarm.d6_switch_proof=1` / `D6_SWITCH_PROOF=1`; Stage 127 now gates initialized x86_64 switch frames on a backed, writable, kernel-only switch-stack page in the target task ASID/root and retries after ASID binding; QEMU proof validation pending local run |
