...
| Metric | Value |
|--------|-------|
| Production `SharedKernel::with_cpu` callsites | **20** |
| Production broad `SharedKernel::with` callsites | **1** |
| **Total production broad-lock acquisition sites** | **21** |
...
2. **Kernel unlocking — canonical Stage 199D.**
   The broad `SpinLock<KernelState>` still has **21** production acquisition sites (§0).
