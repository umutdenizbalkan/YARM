...
| Metric | Value |
|--------|-------|
| Production `SharedKernel::with_cpu` callsites | **33** |
| Production broad `SharedKernel::with` callsites | **6** |
| **Total production broad-lock acquisition sites** | **39** |
...
47 → 44; U3 is in progress and has retired five RISC-V drains, 44 → 42 → 39).
...
2. **Kernel unlocking — canonical Stage 199D.**
   The broad `SpinLock<KernelState>` still has **39** production acquisition sites (§0).
