...
| Metric | Value |
|--------|-------|
| Production `SharedKernel::with_cpu` callsites | **36** |
| Production broad `SharedKernel::with` callsites | **6** |
| **Total production broad-lock acquisition sites** | **42** |
...
(broad-lock callsite census), is documentation rather than lock retirement: 42 callsites
classified as 0 boot-only, 0 test-only, 0 obsolete, 42 runtime-required, 0 undocumented
(U1 deleted the two obsolete acquisitions, 49 → 47; U2 relocated the three test-only ones,
47 → 44; U3 is in progress and has retired two read-only RISC-V drains, 44 → 43 → 42).
...
2. **Kernel unlocking — canonical Stage 199D.**
   The broad `SpinLock<KernelState>` still has **42** production acquisition sites (§0).
