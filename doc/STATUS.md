...
| D6 switch proof | 🧪 default-off `yarm.d6_switch_proof=1` / `D6_SWITCH_PROOF=1`; Stage 128 maps/checks switch-stack pages; Stage 129 adds on-demand active-root repair; Stage 130 fixes TSS RSP0 mismatch after proof switch-back (trampoline now passes `ctx.outgoing_stack_top`) and adds post-`DONE` cleanup markers (`CLEANUP_BEGIN` → `STASH_CLEAR_OK` → `STATE_CLEAR_OK` → `CURRENT_OK` / `CR3_OK` / `TSS_OK` → `CLEANUP_DONE`); QEMU proof validation pending local run |
