...
| Metric | Value |
|--------|-------|
| Production `SharedKernel::with_cpu` callsites | **28** |
| Production broad `SharedKernel::with` callsites | **6** |
| **Total production broad-lock acquisition sites** | **34** |
...
(broad-lock callsite census), is documentation rather than lock retirement: 34 callsites
classified as 0 boot-only, 0 test-only, 0 obsolete, 34 runtime-required, 0 undocumented
(U1 deleted the two obsolete acquisitions, 49 → 47; U2 relocated the three test-only ones,
47 → 44; U3 is in progress and has retired ten drains — six RISC-V, two AArch64, two x86_64 —
44 → 39 → 37 → 36 → 34).
...
2. **Kernel unlocking — canonical Stage 199D.**
   The broad `SpinLock<KernelState>` still has **34** production acquisition sites (§0).
