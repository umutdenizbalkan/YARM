...
| Metric | Value |
|--------|-------|
| Production `SharedKernel::with_cpu` callsites | **17** |
| Production broad `SharedKernel::with` callsites | **1** |
| **Total production broad-lock acquisition sites** | **18** |
...
2. **Kernel unlocking — canonical Stage 199D.**
   The broad `SpinLock<KernelState>` still has **18** production acquisition sites (§0).
