...
| **Total production broad-lock acquisition sites** | **44** |
...
(broad-lock callsite census), is documentation rather than lock retirement: 44 callsites
classified as 0 boot-only, 0 test-only, 0 obsolete, 44 runtime-required, 0 undocumented
(U1 deleted the two obsolete acquisitions, 49 → 47; U2 relocated the three test-only ones,
47 → 44, so the census is now exactly the runtime-required set).
...
2. **Kernel unlocking — canonical Stage 199D.**
   The broad `SpinLock<KernelState>` still has **44** production acquisition sites (§0).
