...
| Metric | Value |
|--------|-------|
| Production `SharedKernel::with_cpu` callsites | **23** |
| Production broad `SharedKernel::with` callsites | **2** |
| **Total production broad-lock acquisition sites** | **25** |
...
2. **Kernel unlocking — canonical Stage 199D.**
   The broad `SpinLock<KernelState>` still has **25** production acquisition sites (§0).
