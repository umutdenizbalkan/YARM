...
The register boundary is deliberately split into retained entry, PC, SP, VBAR, TTBR, TCR, and
printing breadcrumbs. The early hexadecimal writer keeps its conversion loop local rather than
calling the later HH-4 formatting helper; this restores the hardware-proven HH-3 code shape and
makes the last completed operation visible if a future regression occurs.
