...
yarm_server_runtime::install_freestanding_allocator!(
    1024 * 1024,
    "ext4 server freestanding allocator OOM"
);
...
pub extern "C" fn yarm_user_entry() -> ! {
    yarm_user_rt::user_log!("EXT4_SRV_ENTRY");
    yarm_user_rt::user_log!("EXT4_BIN_BEFORE_RUN");
    run();
    yarm_user_rt::user_log!("EXT4_MOUNT_READY");
    let ctx = yarm_server_runtime::user_rt::runtime::startup_context();
    if let Some(recv_cap) = ctx.process_manager_service_recv_ep {
        yarm_user_rt::user_log!("EXT4_SRV_BLOCKING_RECV_LOOP cap={}", recv_cap);
        loop {
...
fn panic(_info: &core::panic::PanicInfo<'_>) -> ! {
    yarm_user_rt::user_log!("EXT4_MOUNT_FAILED panic");
    loop {}
}
