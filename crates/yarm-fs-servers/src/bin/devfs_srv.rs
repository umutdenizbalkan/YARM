...
yarm_server_runtime::install_freestanding_allocator!(
    256 * 1024,
    "devfs server freestanding allocator OOM"
);
...
pub extern "C" fn yarm_user_entry() -> ! {
    yarm_user_rt::user_log!("DEVFS_BIN_ENTRY_START");
    yarm_user_rt::user_log!("DEVFS_BIN_BEFORE_RUN");
    run();
    let ctx = yarm_server_runtime::user_rt::runtime::startup_context();
    if let Some(recv_cap) = ctx.process_manager_service_recv_ep {
        yarm_user_rt::user_log!("DEVFS_SRV_BLOCKING_RECV_LOOP cap={}", recv_cap);
        loop {
