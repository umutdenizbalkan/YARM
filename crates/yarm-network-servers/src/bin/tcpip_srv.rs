...
yarm_server_runtime::install_freestanding_allocator!(
    256 * 1024,
    "tcpip server freestanding allocator OOM"
);
...
pub extern "C" fn yarm_user_entry() -> ! {
    yarm_user_rt::user_log!("TCPIP_BIN_ENTRY_START");
    yarm_user_rt::user_log!("TCPIP_BEFORE_RUN");
    run();
    unreachable!("tcpip server resident service loop returned")
}
