...
yarm_server_runtime::install_freestanding_allocator!(
    256 * 1024,
    "netmgr server freestanding allocator OOM"
);
...
pub extern "C" fn yarm_user_entry() -> ! {
    yarm_user_rt::user_log!("NETMGR_BIN_ENTRY_START");
    yarm_user_rt::user_log!("NETMGR_BEFORE_RUN");
    run();
    unreachable!("netmgr server resident service loop returned")
}
