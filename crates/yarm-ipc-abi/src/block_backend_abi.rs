...
mod tests {
    ...
    #[test] fn short_rejected(){assert!(BlkSgEntry::decode(&[0;23]).is_none());assert!(BlkBackendRequest::decode(&[0;119]).is_none());assert!(BlkBackendResponse::decode(&[0;31]).is_none());}
