...
impl BlkBackendRequest {
    ...
    pub fn is_valid_for_opcode(&self, opcode: u16) -> bool {
        match opcode {
            ...
            BLK_BACKEND_OP_QUERY_STATE | BLK_BACKEND_OP_GET_GEOM | BLK_BACKEND_OP_FLUSH => {
                self.sg_count <= 4
            }
            _ => false,
        }
...
mod tests {
    ...
    #[test]
    fn frozen_opcodes() {
        assert_eq!(BLK_BACKEND_OP_QUERY_STATE, 1);
        assert_eq!(BLK_BACKEND_OP_GET_GEOM, 5);
    }
    #[test]
    fn frozen_status() {
        assert_eq!(BLK_BACKEND_STATUS_EAGAIN, -11);
        assert_eq!(BLK_BACKEND_STATUS_ENOSYS, -38);
    }
    #[test]
    fn sg_roundtrip() {
        let e = BlkSgEntry {
        ...
        assert_eq!(BlkSgEntry::decode(&e.encode()), Some(e));
    }
    #[test]
    fn query_req_roundtrip_and_small() {
        let q = BlkBackendQueryRequest {
            req_id: 1,
            flags: 2,
            device_id: 3,
        };
        assert_eq!(BlkBackendQueryRequest::decode(&q.encode()), Some(q));
        assert!(BlkBackendQueryRequest::ENCODED_LEN < BlkBackendRequest::ENCODED_LEN);
    }
    #[test]
    fn req_roundtrip() {
        let r = BlkBackendRequest {
            req_id: 1,
            flags: 2,
            device_id: 3,
            sector_start: 4,
        ...
        assert_eq!(BlkBackendRequest::decode(&r.encode()), Some(r));
    }
    #[test]
    fn resp_roundtrip() {
        let r = BlkBackendResponse {
            req_id: 1,
            status: BLK_BACKEND_STATUS_EAGAIN,
            actual_bytes: 0,
            backend_generation: 0,
            logical_block_size: 512,
            physical_block_size: 512,
        };
        assert_eq!(BlkBackendResponse::decode(&r.encode()), Some(r));
    }
    #[test]
    fn reject_sg_count_gt4() {
        let mut b = [0u8; BlkBackendRequest::ENCODED_LEN];
        b[28..32].copy_from_slice(&5u32.to_le_bytes());
        assert!(BlkBackendRequest::decode(&b).is_none());
    }
    #[test]
    fn short_rejected() {
        assert!(BlkSgEntry::decode(&[0; 23]).is_none());
        assert!(BlkBackendQueryRequest::decode(&[0; 15]).is_none());
        assert!(BlkBackendRequest::decode(&[0; 127]).is_none());
        assert!(BlkBackendResponse::decode(&[0; 31]).is_none());
    }
    #[test]
    fn read_write_require_sg() {
        let r = BlkBackendRequest {
            req_id: 1,
            flags: 0,
            device_id: 0,
        ...
        assert!(!r.is_valid_for_opcode(BLK_BACKEND_OP_READ));
        assert!(!r.is_valid_for_opcode(BLK_BACKEND_OP_WRITE));
    }
    #[test]
    fn query_allows_zero_sg() {
        let r = BlkBackendRequest {
            req_id: 1,
            flags: 0,
            device_id: 0,
        ...
        assert!(r.is_valid_for_opcode(BLK_BACKEND_OP_QUERY_STATE));
    }

    #[test]
    fn response_golden_vector() {
        let r = BlkBackendResponse {
            req_id: 1,
            status: BLK_BACKEND_STATUS_EAGAIN,
            actual_bytes: 0,
