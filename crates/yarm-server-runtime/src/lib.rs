...
mod freestanding_alloc {
    ...
    pub struct RuntimeBumpAllocator;

    // SAFETY: This allocator uses atomic monotonic bump allocation over a static buffer.
    // It never hands out overlapping ranges and never moves allocated memory.
    unsafe impl GlobalAlloc for RuntimeBumpAllocator {
        unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
            ...
            match result {
                Ok(start) => {
                    // SAFETY: start is proven <= HEAP_SIZE and comes from monotonic bump logic.
                    // add(start) is therefore within HEAP static buffer.
    ...
    fn alloc_error(_layout: Layout) -> ! {
        panic!("yarm-server-runtime freestanding allocator OOM")
    }
}
