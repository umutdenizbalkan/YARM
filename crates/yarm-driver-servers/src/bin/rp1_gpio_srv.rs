...
yarm_server_runtime::install_freestanding_allocator!(
    64 * 1024,
    "rp1 gpio server freestanding allocator OOM"
);

#[inline]
fn run() {
    yarm_user_rt::user_log!("RP1_GPIO_SRV_ENTRY");
    yarm_driver_servers::run_rp1_gpio();
}
...
pub extern "C" fn yarm_user_entry() -> ! {
    yarm_user_rt::user_log!("RP1_GPIO_BIN_ENTRY_START");
    yarm_user_rt::user_log!("RP1_GPIO_BEFORE_RUN");
    run();
    loop {
