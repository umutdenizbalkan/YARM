...
yarm_server_runtime::install_freestanding_allocator!(
    256 * 1024,
    "blkcache server freestanding allocator OOM"
);

#[inline]
fn run() {
    yarm_driver_servers::run_blkcache_srv();
}
...
pub extern "C" fn yarm_user_entry() -> ! {
    yarm_user_rt::user_log!("BLKCACHE_BIN_ENTRY_START");
    yarm_user_rt::user_log!("BLKCACHE_BEFORE_RUN");
    run();
    unreachable!("blkcache server resident service loop returned")
}
