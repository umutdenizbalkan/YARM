...
#[cfg(test)]
fn path_bytes_from_ptr(path_ptr: u64) -> Option<&'static [u8]> {
...
pub fn run_request_loop_over_kernel_ipc(
    ...
    let path = path_bytes_from_ptr(path_ptr).ok_or(VfsError::InvalidPath)?;
...
mod tests {
    ...
    fn vfs_roundtrip_accepts_explicit_zero_tick_recv_budget_when_messages_are_queued() {
        ...
        let reply = roundtrip_ipc_with_budget(
            ...
            server_send,
            client_recv,
            openat_inline_message(0, path_bytes_from_ptr(0x4444).unwrap_or(b"/invalid"), 0, 0)
            .expect("open"),
            0,
        )
        .expect("roundtrip");
    ...
    fn devfs_and_ramfs_conformance_roundtrip_over_kernel_ipc() {
        ...
        let open_dev = roundtrip_ipc(
            ...
            server_send,
            client_recv,
            openat_inline_message(0, path_bytes_from_ptr(DEV_NULL_PATH_PTR).unwrap_or(b"/invalid"), 0, 0)
            .expect("open msg"),
        )
        .expect("open devfs");
        ...
        let open_ram = roundtrip_ipc(
            ...
            server_send,
            client_recv,
            openat_inline_message(0, path_bytes_from_ptr(0xABCD).unwrap_or(b"/invalid"), 0, 0)
            .expect("open msg"),
        )
        .expect("open ramfs");
            ...
            with_kernel_roundtrip(
                ...
                |kernel, initramfs, client_send, server_recv, server_send, client_recv| {
                    let open = roundtrip_ipc(
                        ...
                        server_send,
                        client_recv,
                        openat_inline_message(0, path_bytes_from_ptr(INITRAMFS_BOOT_MARKER_PATH_PTR).unwrap_or(b"/invalid"), 0, 0)
                        .expect("open msg"),
                    )
                ...
                |kernel, devfs, client_send, server_recv, server_send, client_recv| {
                    ...
                    let dev_console_fd = decode_fd_reply(
                        roundtrip_ipc(
                            ...
                            server_send,
                            client_recv,
                            openat_inline_message(0, path_bytes_from_ptr(DEV_CONSOLE_PATH_PTR).unwrap_or(b"/invalid"), 0, 0)
                            .expect("open console"),
                        )
                ...
                |kernel, ramfs, client_send, server_recv, server_send, client_recv| {
                    let ram_fd = decode_fd_reply(
                        roundtrip_ipc(
                            ...
                            server_send,
                            client_recv,
                            openat_inline_message(0, path_bytes_from_ptr(0xCAFE).unwrap_or(b"/invalid"), 0, 0)
                            .expect("open ramfs"),
                        )
                    ...
                    let ram_stat = roundtrip_ipc(
                        ...
                        server_send,
                        client_recv,
                        statx_inline_message(0, path_bytes_from_ptr(0xCAFE).unwrap_or(b"/invalid"), 0, 0)
                        .expect("stat ramfs"),
                    )
