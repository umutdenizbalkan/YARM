...
- 🟡 **Phase 4 — Call/Reply capability model** (Slices 1–3 syscall wiring complete: `IpcCall` + `IpcReply` available; lifecycle hardening in progress: caller-exit reply-cap revocation added).
...
- Slice 4 implementation (partial):
  - reply-cap records are now revoked when the bound caller exits/is reaped to prevent late reply authority reuse.
